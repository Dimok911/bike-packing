  ...
  await context.route("**/*", async route => {
    ...
    if (url.pathname.includes("/letters-vniipo/api/")) {
      ...
      if (["/auth/me", "/auth/experiment-share-session"].includes(suffix)) data = { ok: true, user: { id: "admin-a", email: "admin@example.test" } };
        ...
        capabilities: [...REQUIRED_ADMIN_API_CAPABILITIES, "adminTemplateCausalOperationsV1", "adminTemplateCopyV1", "adminTemplateSourceSaveV1", "adminTemplatePersonalSourceSaveV1", "adminTemplatePendingSourceV1", "adminTemplatePendingPersonalSourceV1", ...(reverseImport ? ["personalListCausalOperationsV1", "personalCausalPhotoPublicationV1", "personalCausalPublicImportV1", "personalCausalPublicEntitiesV1", "personalCausalAdminTemplateImportV1", "personalCausalPendingAdminTemplateImportV1"] : [])] };
        ...
        if (state.privateDispatchWait) await state.privateDispatchWait;
        if (state.privateBlock) return route.abort("failed");
        if (!state.privateReceipts.has(action.operationId) || state.privateReceipts.get(action.operationId).operation.state === "waiting") {
          ...
          if (action.kind === "list.import") {
            ...
            const parentId = manifest.source.base?.operationId, parent = parentId && state.receipts.get(parentId);
            if (parentId && !parent) {
              const waiting = { ok: true, operation: { id: action.operationId, ...binding,
                payloadDigest: createHash("sha256").update(canonicalListOperationJson(binding)).digest("hex"), state: "waiting" },
                result: null, waiting: { code: "dependency_not_committed", operationIds: [parentId], retrySameOperation: true } };
              state.privateReceipts.set(action.operationId, waiting); return route.fulfill({ headers, json: waiting });
            }
            if (parentId) {
              expect(parent.operation.state).toBe("committed"); expect(parent.operation.actorId).toBe(action.expectedActorId);
              expect(parent.operation.listId).toBe(manifest.source.listId); expect(parent.operation.itemKey).toBe(manifest.source.itemKey);
              expect(parent.result.payload.stateRevision).toBe(state.revision);
            } else expect(manifest.source.stateRevision).toBe(state.revision);
            expect(manifest.sourcePayload).toEqual(state.payload);
          } else {
            if (!personalEdits) { expect(state.privateRevision).toBe(4); expect(state.privatePosts[0].kind).toBe("list.import"); }
        ...
        if (!cancel && state.adminDispatchWait) await state.adminDispatchWait;
        ...
        if (!state.receipts.has(id) || state.receipts.get(id).operation.state === "waiting") {
          ...
          const privateParentId = personalProof && intent.body.source.base.operationId;
          const privateParent = privateParentId && state.privateReceipts.get(privateParentId);
          if (privateParentId && !privateParent) {
            const { body, ...identity } = binding;
            const waiting = { operation: { id, ...identity, payloadDigest: createHash("sha256").update(canonicalTemplateJson(binding)).digest("hex"), state: "waiting" },
              result: null, waiting: { code: "template_dependency_not_committed", operationIds: [privateParentId], retrySameOperation: true } };
            state.receipts.set(id, waiting); return route.fulfill({ headers, json: { ok: true, ...waiting } });
          }
          const privateRevision = privateParentId ? privateParent?.operation.state === "committed" && privateParent.operation.listId === "personal-list"
            ? privateParent.result.payload.stateRevision : null : intent.body.source?.base.stateRevision;
          const privateConflict = personalProof && (intent.body.source.listId !== "personal-list" || privateRevision !== state.privateRevision
            || intent.body.source.payloadDigest !== await adminTemplateCopyPayloadDigest(privateParentId ? personalBusinessPayload(state.privatePayload) : state.privatePayload));
...
test.describe("admin reverse personal import", () => {
  ...
  const pendingAdminCases = ["live", "lost-parent", "lost-child", "unknown", "changed-source", "queue-quota"].map(mode => ({ mode, shape: "item" }));
  pendingAdminCases.push({ mode: "live", shape: "tree" });
  for (const shared of [false, true]) for (const { mode, shape } of pendingAdminCases) test(`pending administrative source personal copy ${shared} ${shape} ${mode}`, async ({ page, context }) => {
    const server = await fixture(page, context, { shared, withContainers: true, hydrate: true, personalSource: true, reverseImport: true });
    let releaseAdmin;
    server.adminDispatchWait = new Promise(resolve => { releaseAdmin = resolve; });
    await editItem(page, "Ожидающая правка шаблона", "Насос шаблона");
    await expect.poll(() => server.posts.length).toBe(1);
    const parent = structuredClone(server.posts[0]), privateBefore = personalBusinessPayload(server.privatePayload);
    const before = await page.evaluate(() => {
      const s = __adminUiTest.state(), layout = Object.values(s.layouts).find(row => row.adminCausalSource);
      return { id: layout.id, source: __adminUiTest.snapshot(layout.id),
        item: Object.values(s.items).find(row => row.publicCatalogLayoutId === layout.id && row.name === "Ожидающая правка шаблона").id,
        bag: Object.values(s.containers).find(row => row.publicCatalogLayoutId === layout.id && row.name === "Сумка шаблона").id };
    });
    await page.evaluate(({ before, shape }) => shape === "item" ? __adminUiTest.openItem(before.item) : __adminUiTest.openContainer(before.bag), { before, shape });
    ...
    await page.locator(shape === "item" ? '#containerPickerBoard [data-pick-container="bag"]' : '#containerPickerBoard [data-pick-root-index="0"]').click();
    await expect.poll(() => page.evaluate(() => globalThis.__adminUiLastError || document.querySelector("#confirmDialog").open)).toBe(true);
    if (mode === "changed-source") await page.evaluate(id => { __adminUiTest.state().items[id].name = "Несохранённая поздняя правка"; }, before.item);
    if (mode === "queue-quota") await page.evaluate(() => {
      const set = Storage.prototype.setItem;
      Storage.prototype.setItem = function(key, value) {
        if (key.startsWith("bike-packing-personal-save-v1:") && JSON.parse(value)?.action?.body?.publicImport) {
          throw new DOMException("Pending admin/private queue quota", "QuotaExceededError");
        }
        return set.call(this, key, value);
      };
    });
    server.lose = mode === "lost-parent"; server.privateLose = mode === "lost-child"; server.blockBusiness = mode === "unknown";
    ...
    if (mode === "changed-source") {
      ...
      expect(server.privatePosts).toEqual([]); expect(personalBusinessPayload(server.privatePayload)).toEqual(privateBefore);
      expect(server.posts).toHaveLength(1); expect(server.errors).toEqual([]); return;
    }
    if (mode === "queue-quota") {
      await expect(page.locator("#personalSaveRecoveryDialog")).toBeVisible();
      expect(server.privatePosts).toEqual([]); expect(personalBusinessPayload(server.privatePayload)).toEqual(privateBefore);
    } else await expect.poll(() => page.evaluate(() => Object.entries(localStorage).some(([key, value]) => key.startsWith("bike-packing-personal-save-v1:")
      && JSON.parse(value)?.action?.body?.publicImport?.source?.base?.operationId))).toBe(true);
    const retained = await page.evaluate(() => JSON.parse(Object.entries(localStorage).find(([key]) => key.startsWith("bike-packing-public-selections-v1:") && key.endsWith(":selection"))[1]).selection);
    expect(retained.source.base).toEqual({ operationId: parent.operationId });
    expect(retained.sourcePayload).toEqual(parent.body.payload); releaseAdmin();
    if (mode === "lost-parent") await expect.poll(() => server.hidden).toBe(true);
    if (mode !== "queue-quota") await expect.poll(() => server.privatePosts.length).toBeGreaterThan(0);
    if (mode === "unknown") {
      await expect.poll(() => [...server.privateReceipts.values()][0]?.operation.state).toBe("waiting");
      expect(server.revision).toBe(7); expect(server.privateRevision).toBe(3);
    }
    if (mode === "lost-child") await expect.poll(() => server.privateHidden).toBe(true);
    if (mode === "live") await page.waitForFunction(() => __adminUiTest.privateMeta().stateRevision === 4 && !__adminUiTest.privateMeta().dirty);
    server.lose = false; server.hidden = false; server.privateLose = false; server.privateHidden = false;
    ...
    if (mode === "unknown") {
      ...
      expect(server.posts.every(row => canonicalTemplateJson(row) === canonicalTemplateJson(parent))).toBe(true);
      expect(server.privateRevision).toBe(3); expect(server.revision).toBe(7);
      expect(server.privatePosts.every(row => row.operationId === retained.operationId && row.body.publicImport.source.base.operationId === parent.operationId)).toBe(true);
      expect(await page.evaluate(id => __adminUiTest.snapshot(id), before.id)).toEqual(before.source);
      expect(server.errors).toEqual([]); server.blockBusiness = false;
    }
    const recovery = page.locator("#personalSaveRecoveryDialog");
    ...
    expect(server.posts.every(row => canonicalTemplateJson(row) === canonicalTemplateJson(parent))).toBe(true);
    if (mode !== "unknown") expect(server.posts).toHaveLength(1);
    expect(server.privateReceipts.size).toBe(1); expect(server.revision).toBe(8);
    const action = server.privatePosts.at(-1);
    expect(action.operationId).toBe(retained.operationId); expect(action.body.causal.reads).toEqual([]); expect(action.body.causal.dependsOn).toEqual([]);
    expect(action.body.publicImport.source).toEqual(retained.source);
    expect(server.privatePosts.every(row => canonicalListOperationJson(row) === canonicalListOperationJson(action))).toBe(true);
    expect(action.body.publicImport.ownerTargets).toEqual(retained.ownerTargets);
    expect(action.body.publicImport.ownerTargets).toHaveLength(shape === "item" ? 1 : 3);
    const copiedItem = retained.ownerTargets.find(row => row.entityType === "item");
    expect(server.privatePayload.items[copiedItem.targetId].name).toBe("Ожидающая правка шаблона");
    expect(server.privatePayload.layouts["layout-a"].arrangement.itemQuantities[copiedItem.targetId]).toBe(2);
    expect(personalBusinessPayload(await page.evaluate(() => __adminUiTest.privatePayload()))).toEqual(server.privatePayload);
    expect(server.errors).toEqual([]);
  });
  const pendingPersonalCases = ["live", "lost", "plan-quota", "changed-source", "unknown"].map(mode => ({ mode, shape: "item" }));
  pendingPersonalCases.push({ mode: "live", shape: "tree" });
  for (const shared of [false, true]) for (const { mode, shape } of pendingPersonalCases) test(`pending personal source administrative copy ${shared} ${shape} ${mode}`, async ({ page, context }) => {
    const server = await fixture(page, context, { shared, withContainers: true, hydrate: true, personalSource: true, reverseImport: true, personalEdits: true });
    const target = await page.evaluate(() => {
      const layout = Object.values(__adminUiTest.state().layouts).find(row => row.adminCausalSource);
      return { id: layout.id, before: __adminUiTest.snapshot(layout.id),
        bagId: Object.values(__adminUiTest.state().containers).find(row => row.publicCatalogLayoutId === layout.id && row.name === "Сумка шаблона").id };
    });
    await page.evaluate(() => __adminUiTest.openPrivate("layout-a"));
    let releasePrivate;
    server.privateDispatchWait = new Promise(resolve => { releasePrivate = resolve; });
    await page.evaluate(() => __adminUiTest.openItem("pump")); await page.locator("#itemName").fill("Ожидающая личная правка"); await submitItem(page);
    await expect.poll(() => server.privatePosts.length).toBeGreaterThan(0);
    const parent = structuredClone(server.privatePosts[0]);
    expect(parent.kind).toBe("list.update"); expect(parent.body.payload.activeLayoutId).toBeUndefined(); expect(parent.body.payload.packedItems).toBeUndefined();
    await page.evaluate(shape => shape === "item" ? __adminUiTest.openItem("pump") : __adminUiTest.openContainer("bag"), shape);
    await page.locator(shape === "item" ? "#itemCopyToContainerBtn" : "#rootContainerCopyToContainerBtn").click();
    await page.locator("#containerPickerLayoutSelect").selectOption(target.id);
    if (shape === "item") await page.locator(`#containerPickerBoard [data-pick-container="${target.bagId}"]`).click();
    else await page.locator('#containerPickerBoard [data-pick-root-index="0"]').click();
    await expect(page.locator("#confirmDialog")).toContainText(shape === "item" ? "Скопировать вещь" : "со всем содержимым");
    if (mode === "changed-source") await page.evaluate(() => { __adminUiTest.state().items.pump.name = "Поздняя несохранённая правка"; });
    if (mode === "plan-quota") await page.evaluate(() => {
      const set = Storage.prototype.setItem;
      Storage.prototype.setItem = function(key, value) {
        if (key.startsWith("bike-packing-admin-save-plans-v1:")) throw new DOMException("Pending private/admin plan quota", "QuotaExceededError");
        return set.call(this, key, value);
      };
    });
    server.privateLose = mode === "lost"; server.privateBlock = mode === "unknown";
    await page.locator("#confirmOkBtn").click();
    if (mode === "changed-source") {
      server.privateBlock = true; releasePrivate();
      await expect(page.locator("body")).toContainText("Шаблон или исходный список изменился");
      expect(await page.evaluate(id => __adminUiTest.snapshot(id), target.id)).toEqual(target.before);
      expect(server.posts).toEqual([]); expect(server.privateReceipts.size).toBe(0); expect(server.errors).toEqual([]); return;
    }
    const copiedName = shape === "item" ? "Ожидающая личная правка копия" : "Ожидающая личная правка";
    await expect.poll(() => page.evaluate(({ id, name }) => Object.values(__adminUiTest.snapshot(id).payload.items).some(row => row.name === name), { id: target.id, name: copiedName })).toBe(true);
    releasePrivate();
    if (["lost", "unknown"].includes(mode)) {
      if (mode === "lost") await expect.poll(() => server.privateHidden).toBe(true);
      await expect(page.locator("body")).toContainText("Копия сохранена и ждёт подтверждения личного списка"); expect(server.posts).toEqual([]);
    } else if (mode === "plan-quota") await expect(page.locator("body")).toContainText("Pending private/admin plan quota");
    else await confirmedRevision(page, 8);
    const chosen = await page.evaluate(id => ({ snapshot: __adminUiTest.snapshot(id),
      plan: __adminUiTest.state().layouts[id].adminCausalCopyPlan }), target.id);
    const originalPlanId = chosen.plan?.id || server.posts[0].operationId;
    server.privateLose = false; server.privateHidden = false;
    await page.reload(); await expect(page.locator("body")).toHaveClass(/app-ready/, { timeout: 30000 });
    await page.waitForFunction(() => __adminUiTest.user()?.id === "admin-a");
    if (mode === "unknown") {
      expect(await page.evaluate(id => __adminUiTest.state().layouts[id].adminCausalCopyPlan, target.id)).toEqual(chosen.plan);
      expect(await page.evaluate(id => __adminUiTest.snapshot(id), target.id)).toEqual(chosen.snapshot);
      await page.locator("#syncBtn").click();
      await expect(page.locator("body")).toContainText("Повтор приостановлен, изменения остались на устройстве");
      expect(server.privatePosts).toHaveLength(1); expect(server.privateReceipts.size).toBe(0);
      expect(server.posts).toEqual([]); expect(server.revision).toBe(7); expect(server.privateRevision).toBe(3);
    ...
    expect(server.posts).toHaveLength(1); const action = server.posts[0]; expect(action.operationId).toBe(originalPlanId);
    expect(action.body.source).toEqual({ kind: "personal-list", listId: "personal-list", base: { operationId: parent.operationId },
      payloadDigest: await adminTemplateCopyPayloadDigest(personalBusinessPayload(parent.body.payload)) });
    expect(server.payload).toEqual(stripAdminTemplateEditorMetadata(chosen.snapshot.payload));
    expect(server.privateReceipts.size).toBe(1); expect(server.privatePayload).toEqual(parent.body.payload);
    expect(server.privatePosts.every(row => canonicalListOperationJson(row) === canonicalListOperationJson(parent))).toBe(true);
    if (shape === "tree") {
      const added = Object.values(server.payload.items).filter(row => !target.before.payload.items[row.id]);
      expect(added).toHaveLength(1);
      const arrangement = Object.values(server.payload.layouts)[0].arrangement;
      expect(arrangement.itemQuantities[added[0].id]).toBe(2);
      expect(arrangement.packedItems[added[0].id]).toBeUndefined();
      expect(Object.keys(server.payload.containers)).toHaveLength(Object.keys(target.before.payload.containers).length + 2);
    }
    await editItem(page, "Следующая правка после ожидающего источника", copiedName, target.id); await confirmedRevision(page, 9);
    expect(server.posts[1].body.source).toBeUndefined(); expect(server.posts[1].body.base).toEqual({ stateRevision: 8 }); expect(server.errors).toEqual([]);
  });
  for (const shared of [false, true]) for (const quota of [false, true]) test(`personal outbox administrative copy survives reload ${shared} quota=${quota}`, async ({ page, context }) => {
    const server = await fixture(page, context, { shared, withContainers: true, hydrate: true, personalSource: true, reverseImport: true, personalEdits: true });
    const target = await page.evaluate(() => {
      const s = __adminUiTest.state(), layout = Object.values(s.layouts).find(row => row.adminCausalSource);
      const bag = Object.values(s.containers).find(row => row.publicCatalogLayoutId === layout.id && row.name === "Сумка шаблона");
      __adminUiTest.openPrivate("layout-a"); return { id: layout.id, bagId: bag.id };
    });
    await page.evaluate(() => __adminUiTest.openItem("pump"));
    await page.locator("#itemName").fill("Личная правка перед копией"); await submitItem(page);
    await expect.poll(() => server.privatePosts.length, { timeout: 30000 }).toBe(1);
    await page.waitForFunction(() => __adminUiTest.privateMeta().stateRevision === 4 && !__adminUiTest.privateMeta().dirty);
    const privateBefore = structuredClone(server.privatePayload);
    await page.evaluate(() => __adminUiTest.openItem("pump")); await page.locator("#itemCopyToContainerBtn").click();
    await page.locator("#containerPickerLayoutSelect").selectOption(target.id);
    await page.locator(`#containerPickerBoard [data-pick-container="${target.bagId}"]`).click();
    await expect(page.locator("#confirmDialog")).toContainText("Скопировать вещь");
    if (quota) await page.evaluate(() => {
      const set = Storage.prototype.setItem;
      Storage.prototype.setItem = function(key, value) {
        if (key.startsWith("bike-packing-admin-save-plans-v1:")) throw new DOMException("Private/admin plan quota", "QuotaExceededError");
        return set.call(this, key, value);
      };
    });
    await page.locator("#confirmOkBtn").click();
    await expect.poll(() => page.evaluate(id => Object.values(__adminUiTest.snapshot(id).payload.items).some(item => item.name === "Личная правка перед копией копия"), target.id)).toBe(true);
    if (quota) {
      await expect.poll(() => page.evaluate(id => Boolean(__adminUiTest.state().layouts[id].adminCausalCopyPlan), target.id)).toBe(true);
      expect(server.posts).toHaveLength(0);
    } else await confirmedRevision(page, 8);
    const chosen = await page.evaluate(id => __adminUiTest.snapshot(id), target.id);
    await page.reload(); await expect(page.locator("body")).toHaveClass(/app-ready/, { timeout: 30000 });
    await page.waitForFunction(() => __adminUiTest.user()?.id === "admin-a");
    await page.evaluate(shared => __adminUiTest.openPrepared(shared ? { type: "shared", sharedId: "ui" }
      : { type: "demo", demoListId: "public-demo-state-ui", language: "ru" }), shared);
    await confirmedRevision(page, 8);
    expect(server.posts).toHaveLength(1); expect(server.privatePosts).toHaveLength(1);
    expect(server.payload).toEqual(stripAdminTemplateEditorMetadata(chosen.payload));
    expect(server.privatePayload).toEqual(privateBefore); expect(server.errors).toEqual([]);
  });
...
for (const sourceShared of [false, true]) for (const shape of ["item", "tree"]) for (const mode of ["confirmed", "lost", "cancel", "changed-source", "plan-quota", "source-conflict", "publication", "publication-lost", "hiding", "hiding-lost", "metadata", "metadata-lost", "metadata-language", "metadata-root", "metadata-root-lost", "metadata-root-language", "metadata-confirmed-data", "unpublish-root", "unpublish-root-lost", "unpublish-afterdata", "archive-root", "archive-root-lost", "archive-afterdata"]) {
  test(`admin pending source ${sourceShared ? "shared" : "demo"} ${shape} (${mode})`, async ({ page, context }) => {
    const server = await crossTemplateFixture(page, context, sourceShared, !sourceShared, false, false, mode.startsWith("hiding") || mode.startsWith("unpublish")), target = server.target;
    ...
    const command = mode.startsWith("unpublish") || mode.startsWith("archive"), rootCommand = command && mode.includes("-root");
    if (mode === "metadata-confirmed-data") server.blockBusiness = false;
    const sourceName = publication || rootMetadata || rootCommand ? "Насос шаблона" : "Ожидающая правка источника";
    if (publication || hiding) await capturePendingSourceVisibility(page, server, sourceShared, hiding, sourceName);
    else if (!rootMetadata && !rootCommand) await editItem(page, sourceName, "Насос шаблона");
    if (command) await capturePendingStandaloneCommand(page, mode.startsWith("archive"));
    ...
    await confirmedRevision(page, mode === "unpublish-afterdata" ? 10 : !rootMetadata && !rootCommand && (publication || hiding || command || mode.startsWith("metadata")) ? 9 : 8);
...
async function newDraftFixture(page, context, options = {}) {
  ...
  await context.route("**/bike-packing/admin/template-operations**", async route => {
    ...
    if (url.pathname.endsWith("/prepare")) {
      const { itemKey } = request.postDataJSON(); if (typeof itemKey !== "string") return route.fallback();
      const shared = itemKey.startsWith("shared-layout:");
      const listId = shared ? "public-shared-layout-" + itemKey.slice("shared-layout:".length) : "public-demo-state-" + itemKey.slice("demo-state:".length);
      const row = created.get(listId); if (!row) return route.fallback();
      return route.fulfill({ headers, json: { ok: true, actorId: "admin-a", environment: "bike-packing-experiment", itemKey, listId,
        sourceType: shared ? "curated-bikepacker" : "public-template", exists: true, deleted: false, stateRevision: row.revision,
        visibility: "private", metadata: row.metadata, payload: row.payload, indexes: [] } });
    }
    ...
    if (state.blockNew && ["template.copy", "template.create"].includes(input.kind)) return route.abort("failed");
    if (!state.receipts.has(id) || state.receipts.get(id).operation.state === "waiting") {
      if (input.kind === "template.copy") {
        ...
        if (input.body.source.base.operationId && (!sourceParent || sourceParent.operation.state === "waiting")) {
          const { body, ...identity } = binding;
          state.receipts.set(id, { operation: { id, ...identity, payloadDigest: createHash("sha256").update(canonicalTemplateJson(binding)).digest("hex"), state: "waiting" },
            result: null, waiting: { code: "template_dependency_not_committed", operationIds: [input.body.source.base.operationId], retrySameOperation: true } });
      ...
      if (!["template.create", "template.copy"].includes(input.kind)) expect(input.body.base.stateRevision ?? state.receipts.get(input.body.base.operationId)?.result?.payload.stateRevision).toBe(row.revision);
      if (input.kind === "template.metadata") Object.assign(row.metadata, input.body.metadata);
      if (input.kind === "template.save") row.payload = structuredClone(input.body.payload);
      if (input.kind === "template.archive") { row.visibility = "private"; row.archived = true; }
      row.revision++;
      const { body, ...identity } = binding;
      state.receipts.set(id, { operation: { id, ...identity, payloadDigest: createHash("sha256").update(canonicalTemplateJson(binding)).digest("hex"), state: "committed" },
...
const newPendingTargetCases = [];
for (const shared of [false, true]) for (const kind of ["create", "copy"]) for (const shape of kind === "create" ? ["tree"] : ["item", "tree"])
  for (const mode of ["confirmed", "lost-parent", "plan-quota", "live", ...(shape === "tree" ? ["cancel", "changed-target"] : []),
    ...(kind === "copy" && shape === "tree" ? ["fresh-editor", "legacy-identity"] : [])]) newPendingTargetCases.push({ shared, kind, shape, mode });
for (const { shared, kind, shape, mode } of newPendingTargetCases) {
  test(`new pending admin target ${shared ? "shared" : "demo"} ${kind} ${shape} (${mode})`, async ({ page, context }) => {
    const server = await newDraftFixture(page, context, { shared, withContainers: true, hydrate: true });
    const sourceId = await page.evaluate(() => __adminUiTest.state().activeLayoutId);
    const sourceSelector = shared ? { type: "shared", sharedId: "ui" } : { type: "demo", demoListId: "public-demo-state-ui", language: "ru" };
    server.blockNew = mode !== "lost-parent"; server.createLose = mode === "lost-parent";
    if (kind === "create") await submitNewAdminDraft(page, shared ? "shared" : "demo", "Новая ожидающая цель");
    else await submitAdminTemplateCopy(page, "Новая ожидающая цель", sourceId);
    await expect(page.locator("#layoutDialog")).not.toBeVisible();
    await expect.poll(() => server.posts.filter(post => post.kind === "template." + kind).length).toBeGreaterThan(0);
    const parent = structuredClone(server.posts.find(post => post.kind === "template." + kind));
    if (mode === "lost-parent") await expect.poll(() => server.createHidden.has(parent.operationId)).toBe(true);
    else expect(server.created.has(parent.listId)).toBe(false);
    const targetId = await page.evaluate(listId => Object.values(__adminUiTest.state().layouts).find(row => row.adminCausalSource?.binding.listId === listId).id, parent.listId);
    await expect.poll(() => page.evaluate(id => __adminUiTest.state().layouts[id].adminCausalSource.planId, targetId)).toBe(parent.operationId);
    if (mode === "legacy-identity") {
      const legacy = await page.evaluate(({ targetId, parentId }) => {
        delete __adminUiTest.state().layouts[targetId].sharedSourceId;
        const [key, value] = Object.entries(localStorage).find(([key, value]) => key.startsWith("bike-packing-admin-save-plans-v1:") && JSON.parse(value).plan.id === parentId);
        return { key, row: JSON.parse(value), snapshot: __adminUiTest.snapshot(targetId) };
      }, { targetId, parentId: parent.operationId });
      legacy.snapshot.payload = stripAdminTemplateEditorMetadata(legacy.snapshot.payload); legacy.row.plan.editorSnapshot = legacy.snapshot;
      legacy.row.digest = createHash("sha256").update(canonicalTemplateJson(legacy.row.plan)).digest("hex");
      await page.evaluate(({ key, row }) => localStorage.setItem(key, JSON.stringify(row)), legacy);
    }
    const targetSelector = parent.itemKey.startsWith("shared-layout:") ? { type: "shared", sharedId: parent.itemKey.slice("shared-layout:".length) }
      : { type: "demo", demoListId: parent.listId, language: "ru" };
    const originalTarget = kind === "create" ? structuredClone(parent.body.payload) : projectAdminTemplateCopy(server.payload, parent.operationId, parent.body.metadata);
    const originalSource = structuredClone(server.payload), before = await page.evaluate(({ sourceId, targetId }) => {
      const current = __adminUiTest.state(); return { sourceId, targetId, target: __adminUiTest.snapshot(targetId),
        item: Object.values(current.items).find(row => row.publicCatalogLayoutId === sourceId).id,
        bag: current.layouts[sourceId].rootContainerIds[0], targetBag: current.layouts[targetId].rootContainerIds[0] };
    }, { sourceId, targetId });
    await page.evaluate(value => __adminUiTest.openPrepared(value), sourceSelector);
    await page.evaluate(({ shape, before }) => shape === "item" ? __adminUiTest.openItem(before.item) : __adminUiTest.openContainer(before.bag), { shape, before });
    await page.locator(shape === "item" ? "#itemCopyToContainerBtn" : "#rootContainerCopyToContainerBtn").click();
    ...
    await page.locator(shape === "item" ? `#containerPickerBoard [data-pick-container="${before.targetBag}"]` : '#containerPickerBoard [data-pick-root-index="0"]').click();
    if (mode === "legacy-identity") {
      await expect(page.locator("body")).toContainText("Новая копия использует прежние идентификаторы редактора");
      await expect(page.locator("#confirmDialog")).not.toBeVisible();
      expect(server.posts.every(post => post.operationId === parent.operationId)).toBe(true);
      expect(server.created.has(parent.listId)).toBe(false); expect(server.payload).toEqual(originalSource); expect(server.errors).toEqual([]); return;
    }
    await expect(page.locator("#confirmDialog")).toBeVisible();
    if (mode === "changed-target") await page.evaluate(id => { __adminUiTest.state().layouts[id].name = "Изменённая новая цель"; }, targetId);
    if (mode === "plan-quota") await page.evaluate(parentId => {
      const set = Storage.prototype.setItem;
      Storage.prototype.setItem = function(key, value) {
        if (key.startsWith("bike-packing-admin-save-plans-v1:") && JSON.parse(value).plan.id !== parentId) throw new DOMException("New target plan quota", "QuotaExceededError");
        return set.call(this, key, value);
      };
    }, parent.operationId);
    await page.locator(mode === "cancel" ? "#confirmCancelBtn" : "#confirmOkBtn").click();
    if (["cancel", "changed-target"].includes(mode)) {
      if (mode === "changed-target") await expect(page.locator("body")).toContainText("Шаблон изменился");
      const current = await page.evaluate(id => ({ source: __adminUiTest.state().layouts[id].adminCausalSource, snapshot: __adminUiTest.snapshot(id) }), targetId);
      expect(current.source.planId).toBe(parent.operationId);
      for (const type of ["items", "containers"]) expect(current.snapshot.payload[type]).toEqual(before.target.payload[type]);
      expect(server.posts.every(post => post.operationId === parent.operationId)).toBe(true);
      expect(server.payload).toEqual(originalSource); expect(server.errors).toEqual([]); return;
    }
    await expect.poll(() => page.evaluate(({ id, parentId, mirrored }) => {
      const layout = __adminUiTest.state().layouts[id]; return mirrored ? Boolean(layout.adminCausalCopyPlan) : layout.adminCausalSource.planId !== parentId;
    }, { id: targetId, parentId: parent.operationId, mirrored: mode === "plan-quota" })).toBe(true);
    const captured = await page.evaluate(id => {
      const layout = __adminUiTest.state().layouts[id]; return { snapshot: __adminUiTest.snapshot(id), plan: layout.adminCausalCopyPlan
        || JSON.parse(Object.entries(localStorage).find(([key, value]) => key.startsWith("bike-packing-admin-save-plans-v1:") && JSON.parse(value).plan.id === layout.adminCausalSource.planId)[1]).plan };
    }, targetId);
    expect(captured.plan.version).toBe(4); expect(captured.plan.operations[0].body.base).toEqual({ operationId: parent.operationId });
    expect(server.posts.every(post => post.operationId === parent.operationId)).toBe(true);
    if (mode === "live") {
      await page.locator("#syncBtn").click(); const recovery = page.locator("#adminTemplateRecoveryDialog");
      await expect(recovery.locator("[data-admin-resume]")).toBeEnabled(); server.blockNew = false;
      await recovery.locator("[data-admin-resume]").click(); await confirmedRevision(page, 2);
      await expect(recovery.locator("[data-admin-recovery-close]")).toBeEnabled(); await recovery.locator("[data-admin-recovery-close]").click();
    } else {
      server.blockNew = false; server.createLose = false; server.createHidden.clear();
      await page.reload(); await expect(page.locator("body")).toHaveClass(/app-ready/, { timeout: 30000 });
      await page.waitForFunction(() => window.__adminUiTest?.user()?.id === "admin-a");
      await page.evaluate(value => __adminUiTest.openPrepared(value), targetSelector); await confirmedRevision(page, 2);
    }
    const actual = server.created.get(parent.listId); expect(actual.revision).toBe(2);
    expect(actual.payload).toEqual(stripAdminTemplateEditorMetadata(captured.snapshot.payload));
    expect(Object.keys(actual.payload.layouts)).toEqual(Object.keys(originalTarget.layouts));
    for (const type of ["items", "containers"]) for (const id of Object.keys(originalTarget[type])) {
      const expected = before.target.payload[type][id], current = actual.payload[type][id];
      expect(expected).toBeTruthy(); expect(current).toBeTruthy();
      if (type === "containers" && shape === "item" && id === before.targetBag) {
        const { itemIds, order, ...rest } = expected, { itemIds: nextItems, order: nextOrder, ...nextRest } = current;
        expect(nextRest).toEqual(rest); expect(nextItems.filter(key => itemIds.includes(key))).toEqual(itemIds);
        expect(nextOrder.filter(row => order.some(old => old.type === row.type && old.id === row.id))).toEqual(order);
      } else expect(current).toEqual(expected);
    }
    expect(server.payload).toEqual(originalSource);
    for (const post of server.posts) {
      expect([parent.operationId, captured.plan.id]).toContain(post.operationId);
      expect(post.body).toEqual(post.operationId === parent.operationId ? parent.body : captured.plan.operations[0].body);
    }
    if (mode === "live") { await editItem(page, "Правка после новой цели", "Насос шаблона", targetId); await confirmedRevision(page, 3); expect(server.posts.at(-1).body.base).toEqual({ stateRevision: 2 }); }
    if (mode === "fresh-editor") {
      const ids = Object.fromEntries(["items", "containers", "layouts"].map(type => [type, Object.keys(actual.payload[type]).sort()]));
      await page.evaluate(() => localStorage.clear()); await page.reload(); await expect(page.locator("body")).toHaveClass(/app-ready/, { timeout: 30000 });
      await page.waitForFunction(() => window.__adminUiTest?.user()?.id === "admin-a");
      await page.evaluate(value => __adminUiTest.openPrepared(value), targetSelector); await confirmedRevision(page, 2);
      const freshId = await page.evaluate(() => __adminUiTest.state().activeLayoutId);
      await editItem(page, "Правка в новом редакторе копии", "Насос шаблона", freshId); await confirmedRevision(page, 3);
      for (const type of ["items", "containers", "layouts"]) expect(Object.keys(actual.payload[type]).sort()).toEqual(ids[type]);
      expect(server.posts.at(-1).body.base).toEqual({ stateRevision: 2 });
    }
    expect(server.errors).toEqual([]);
  });
}
...
for (const shared of [false, true]) for (const pending of [false, true]) for (const change of ["name", "mode", "closed"]) {
  test(`whole admin copy form ${shared ? "shared" : "demo"} ${pending ? "pending" : "confirmed"} cancels changed ${change}`, async ({ page, context }) => {
    const state = await newDraftFixture(page, context, { shared, withContainers: true });
    if (pending) {
      state.blockBusiness = true; await editItem(page, "Ожидающая версия формы", "Насос шаблона");
      await expect.poll(() => state.posts.length).toBeGreaterThan(0);
    }
    const before = await page.evaluate(() => {
      const current = __adminUiTest.state(), source = Object.values(current.layouts).find(row => row.adminCausalSource);
      return { ids: Object.keys(current.layouts), sourceId: source.id, snapshot: __adminUiTest.snapshot(source.id) };
    });
    let release, preparing = false;
    if (pending) await page.evaluate(() => {
      const original = crypto.subtle.digest.bind(crypto.subtle);
      const gate = new Promise(resolve => { globalThis.__releaseWholeCopyDigest = resolve; });
      globalThis.__wholeCopyDigestWaiting = false;
      crypto.subtle.digest = async (...args) => {
        if (document.querySelector("#layoutDialog")?.open && document.querySelector("#saveLayoutBtn")?.disabled
          && document.querySelector("#layoutCreateMode")?.value === "template-copy") {
          globalThis.__wholeCopyDigestWaiting = true; await gate;
        }
        return original(...args);
      };
    });
    else state.onAdminPrepare = () => { preparing = true; return new Promise(resolve => { release = resolve; }); };
    await submitAdminTemplateCopy(page, "Копия с изменённой формой");
    if (pending) await expect.poll(() => page.evaluate(() => globalThis.__wholeCopyDigestWaiting)).toBe(true);
    else await expect.poll(() => preparing).toBe(true);
    if (change === "name") await page.locator("#layoutName").fill("Другое имя копии");
    else if (change === "mode") await page.locator("#layoutCreateMode").selectOption("empty");
    else await page.locator("#layoutDialog").getByRole("button", { name: "Закрыть", exact: true }).click();
    if (pending) await page.evaluate(() => globalThis.__releaseWholeCopyDigest()); else release();
    await expect(page.locator("#saveLayoutBtn")).toBeEnabled();
    if (change === "closed") await expect(page.locator("#layoutDialog")).not.toBeVisible();
    else await expect(page.locator("#layoutDialog")).toBeVisible();
...
for (const shared of [false, true]) for (const shape of ["whole", "item", "tree"]) for (const mode of ["confirmed", "lost", "plan-quota", "source-conflict", "metadata", "metadata-language", "metadata-confirmed-copy", "archive"]) {
  test(`pending copied source ${shared ? "shared" : "demo"} ${shape} (${mode})`, async ({ page, context }) => {
    ...
    if (mode === "archive") await capturePendingStandaloneCommand(page, true);
    const before = await page.evaluate(({ parentList, targetList }) => {
      ...
      return { sourceId: source.id, targetId: target.id, source: __adminUiTest.snapshot(source.id), target: __adminUiTest.snapshot(target.id), parentId: source.adminCausalSource.base.operationId,
    ...
    else await confirmedRevision(page, mode.startsWith("metadata") || mode === "archive" ? 2 : 1);
    ...
    if (mode === "archive") expect(state.created.get(parent.listId).archived).toBe(true);
...
for (const shared of [false, true]) for (const mode of ["confirmed", "lost", "plan-quota", "conflict", "inactive", "publication", "publication-lost", "hiding", "hiding-lost", "metadata", "metadata-lost", "metadata-language", "metadata-root", "metadata-root-lost", "metadata-root-language", "metadata-confirmed-data", "unpublish-root", "unpublish-root-lost", "unpublish-afterdata", "archive-root", "archive-root-lost", "archive-afterdata"]) {
  test(`whole pending admin template ${shared ? "shared" : "demo"} (${mode})`, async ({ page, context }) => {
    const state = await newDraftFixture(page, context, { shared, published: mode.startsWith("hiding") || mode.startsWith("unpublish"), withContainers: true, catalogPair: true, layoutOrder: 17 });
    state.blockBusiness = true;
    ...
    else if (!rootMetadata && !rootCommand) await editItem(page, "Ожидающая целая копия", "Насос шаблона");
    if (command) await capturePendingStandaloneCommand(page, mode.startsWith("archive"));
    ...
    if (mode === "inactive") await page.evaluate(() => __adminUiTest.openPrivate("layout-a"));
    if (mode === "plan-quota") await page.evaluate(() => {
      const set = Storage.prototype.setItem;
      Storage.prototype.setItem = function(key, value) {
        if (key.startsWith("bike-packing-admin-save-plans-v1:") && value.includes('"kind":"template.copy"')) throw new DOMException("Pending whole copy quota", "QuotaExceededError");
        return set.call(this, key, value);
      };
    });
    await submitAdminTemplateCopy(page, "Копия ожидающего шаблона");
    await expect(page.locator("#layoutDialog")).not.toBeVisible();
    const copy = await page.evaluate(() => Object.values(__adminUiTest.state().layouts).find(row => row.name === "Копия ожидающего шаблона"));
    expect(copy).toBeTruthy(); const binding = copy.adminCausalSource.binding;
    const posts = () => state.posts.filter(post => post.kind === "template.copy");
    let action;
    if (mode === "plan-quota") {
      const { id, actorId, ...wire } = copy.adminCausalCopyPlan.operations[0];
      action = { ...wire, expectedActorId: actorId, operationId: id }; expect(posts()).toEqual([]);
    } else {
      await expect.poll(() => posts().length).toBe(1); action = structuredClone(posts()[0]);
      expect(state.receipts.get(action.operationId).operation.state).toBe("waiting");
    }
    expect(action.body.source.base).toEqual({ operationId: before.parentId });
    expect(state.created.size).toBe(0); expect(state.payload).toEqual(sourceBefore);
    state.blockBusiness = false;
    await page.reload(); await expect(page.locator("body")).toHaveClass(/app-ready/, { timeout: 30000 });
    await page.waitForFunction(() => __adminUiTest.user()?.id === "admin-a");
    await page.evaluate(shared => __adminUiTest.openPrepared(shared ? { type: "shared", sharedId: "ui" }
    ...
    if (mode === "conflict") {
      expect(state.receipts.get(action.operationId).operation.state).toBe("rejected"); expect(state.created.size).toBe(0);
    } else {
      await confirmedRevision(page, 1); expect(state.created.size).toBe(1);
      expect(state.created.get(binding.listId).payload).toEqual(expected);
      const current = await page.evaluate(id => __adminUiTest.state().layouts[id], copy.id);
      expect(current.templatePublished).toBe(false); expect(current.layoutOrder).toBe(17);
    }
    expect(posts()).toHaveLength(mode === "plan-quota" ? 1 : 2); for (const post of posts()) expect(post).toEqual(action);
    const sourceAfter = await page.evaluate(id => __adminUiTest.snapshot(id), before.id);
    expect(sourceAfter.metadata).toEqual(before.snapshot.metadata);
    expect(stripAdminTemplateEditorMetadata(sourceAfter.payload)).toEqual(stripAdminTemplateEditorMetadata(before.snapshot.payload));
    expect(state.errors).toEqual([]);
  });
}
...
for (const shared of [false, true]) test(`whole admin template copy ${shared ? "shared" : "demo"} cannot use an unsaved edit beyond its pending source`, async ({ page, context }) => {
  ...
  await page.evaluate(() => { Object.values(__adminUiTest.state().items).find(row => row.publicCatalogLayoutId).name = "Ещё не записанная правка"; });
  const before = state.posts.length;
  await submitAdminTemplateCopy(page, "Копия неподтверждённого источника");
  await expect(page.locator("#layoutDialog")).toBeVisible(); await expect(page.locator("body")).toContainText("не совпадает с сохранённым действием");
...
async function capturePendingStandaloneCommand(page, archive) {
  const previous = await page.evaluate(() => __adminUiTest.state().layouts[__adminUiTest.state().activeLayoutId].adminCausalSource.planId);
  await page.getByRole("button", { name: "Редактировать текущую укладку", exact: true }).click();
  await page.locator(archive ? "#deleteEditedLayoutBtn" : "#publishEditedTemplateBtn").click();
  await expect(page.locator("#confirmDialog")).toBeVisible(); await page.locator("#confirmOkBtn").click();
  await expect.poll(() => page.evaluate(() => __adminUiTest.state().layouts[__adminUiTest.state().activeLayoutId].adminCausalSource.planId)).not.toBe(previous);
  await expect(page.locator("body")).toContainText(archive ? "Не удалось удалить шаблон:" : "сервер не подтвердил снятие с публикации");
  if (await page.locator("#layoutEditDialog").isVisible()) {
    await page.locator("#layoutEditDialog").getByRole("button", { name: "Закрыть", exact: true }).click();
    await expect.poll(async () => await page.locator("#layoutEditDialog").isVisible()
      ? await page.locator("#confirmDialog").isVisible() ? "confirm" : "waiting" : "closed").not.toBe("waiting");
    if (await page.locator("#confirmDialog").isVisible()) await page.locator("#confirmCancelBtn").click();
  }
...
async function renameTemplate(page, name, language = "") {
  await page.getByRole("button", { name: "Редактировать текущую укладку", exact: true }).click();
  ...
  if (language) await page.locator("#layoutEditLanguage").selectOption(language);
...
async function capturePendingSourceLabel(page, name, language = "") {
  const previous = await page.evaluate(() => __adminUiTest.state().layouts[__adminUiTest.state().activeLayoutId].adminCausalSource.planId);
  await renameTemplate(page, name, language);
  await expect.poll(() => page.evaluate(() => __adminUiTest.state().layouts[__adminUiTest.state().activeLayoutId].adminCausalSource.planId)).not.toBe(previous);
  await expect(page.locator("#saveEditedLayoutBtn")).toBeEnabled();
  await page.locator("#layoutEditDialog").getByRole("button", { name: "Закрыть", exact: true }).click();
  await expect(page.locator("#confirmDialog")).toBeVisible();
