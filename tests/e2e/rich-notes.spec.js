...
test('new bags and items save rich notes and preserve them in the form draft', async({page,isMobile})=>{
  ...
  await activate(page.locator('[data-add-packing-root]'),isMobile);
  await activate(page.locator('#createRootForLayoutBtn'),isMobile);
  ...
  await activate(bag.locator('[data-add-to-container]'),isMobile);
  await activate(page.locator('#createItemForContainerBtn'),isMobile);
