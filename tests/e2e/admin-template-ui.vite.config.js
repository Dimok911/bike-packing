...
export default defineConfig({
  ...
  plugins: [{ name: "isolated-admin-template-ui", enforce: "pre", transform(code, id) {
    ...
    if (source.endsWith("/app.js")) return code + "\nwindow.__adminUiTest={openDemo:openAdminDemoLayout,openShared:openSharedLayoutForAdmin,openItem:openItemDialog,save:savePublishedLayoutRecord,state:()=>state,user:()=>currentUser,scope:()=>currentViewScope()};\n";
  } }],
