...
export default defineConfig(({ mode }) => ({
  ...
  plugins: [{ name: "isolated-personal-save-pilot", enforce: "pre", transform(code, id) {
    ...
    if (mode === "photo-form" && /\/src\/sync\/personal-photo-form-(protocol|gates)\.js$/.test(source)) {
      return code.replace(/(PERSONAL_PHOTO_FORM(?:_UI)?_ENABLED) = false/g, "$1 = true");
    }
    if (["photo-recovery", "photo-form"].includes(mode) && /\/src\/sync\/personal-photo-(action-store|outbox-record|publication-protocol|staging|batch-cancellation)\.js$/.test(source)) {
    ...
    if (/\/src\/sync\/(personal-save-outbox|list-operation-queue)\.js$/.test(id.replaceAll("\\", "/"))) {
      ...
      return ["photo-recovery", "photo-form"].includes(mode) ? gated.replace("LIST_OPERATION_CANCELLATION_ENABLED = false", "LIST_OPERATION_CANCELLATION_ENABLED = true") : gated;
    }
  } }],
  build: { outDir: mode === "photo-form" ? "test-results/personal-photo-form-ui-build" : mode === "photo-recovery" ? "test-results/personal-photo-cancel-ui-build" : "test-results/personal-ui-build", emptyOutDir: true }
}));
