...
import { personalServerPhotoResultReference, isPersonalPendingServerUpdate, personalPendingServerUpdateSource } from "../../src/sync/personal-pending-server-update.js";

async function fixture(fileless = false) {
  const f = await serverImportFixture(fileless);
...
const allowed = (source, basePayload, payload, userDeletion = null) => isPersonalPendingServerUpdate({ source, basePayload, payload, userDeletion, listId: "list" });
const child = (source, parent, payload, userDeletion) => ({ version: 1, snapshot: structuredClone(payload), action: { ...parent.action,
  ...
  body: { baseStateRevision: 7, payload, ...(userDeletion ? { userDeletion } : {}), photoResults: personalServerPhotoResultReference(source),
...
for (const fileless of [false, true]) test(`pending server ${fileless ? "fileless" : "photo"} fields and explicit deletion follow both exact parents without reviving owners`, async () => {
  ...
  assert.equal(personalPendingServerUpdateSource({ records: [third, source, second, first], operationId: third.action.operationId, listId: "list" }), source);
  assert.equal(personalServerPhotoResultReference(source).version, 12);
  assert.equal(allowed(source, deletion.snapshot, edited), false);
  assert.deepEqual(source, frozen);
});

test("pending server refuses missing/rebound/reordered files, unproved owner deletion and archive substitution", async () => {
  ...
  const archive = structuredClone(source); archive.action.body.archiveImport = archive.action.body.serverImport; delete archive.action.body.serverImport;
  assert.equal(allowed(archive, base, base), false);
});

test("pending server layout removal keeps owners and all selected photos", async () => {
...
test("pending server ancestry rejects wrong source versions, omitted parents, cycles and foreign actors", async () => {
  ...
  assert.equal(personalPendingServerUpdateSource({ records: [source], operationId: source.action.operationId, listId: "list", includeSource: true }), source);
  for (const mutate of [r => r.action.body.photoResults.version = 3, r => r.action.body.causal.dependsOn.pop(), r => r.action.actorId = "foreign",
    ...
    assert.equal(personalPendingServerUpdateSource({ records: [source, first, changed], operationId: changed.action.operationId, listId: "list" }), null);
  }
});

for (const fileless of [false, true]) for (const quota of [false, true]) test(`server pending form captures before view and cold restart, fileless=${fileless}, quota=${quota}`, async () => {
  const f = await serverImportFixture(fileless), values = new Map(), native = new Map(); let failWrites = false, views = 0;
  ...
  const store = { binding: f.binding, ids: async () => [...native.keys()], read: id => decodePersonalServerImportRecord(native.get(id), f.binding, id),
    captureServer: async value => native.set(value.action.operationId, await encodePersonalServerImportRecord({ binding: f.binding, ...value })) };
  const prepared = await preparePersonalServerImport({ selection: f.selection, enabled: true, outbox, store, getContext,
    selectionStore: createPersonalServerImportSelectionStore({ binding: f.binding, storage, locks, getContext, enabled: true }),
  ...
  const session = createPersonalPendingServerFormSession({ enabled: true, outbox, getContext, onDurable: record => {
  ...
  if (quota) {
    ...
    assert.equal(saved.action.body.photoResults.version, 12); assert.equal(saved.action.body.photoResults.operationId, source.action.operationId);
  ...
  if (!fileless) {
    ...
    assert.deepEqual(inventory.entries[0].stageOperationIds, f.action.body.serverImport.files.map(file => file.assetId));
  }
});
