...
function fixture({ enabled = true, editEnabled = false, actor = "admin-a", personal = false, pending = null } = {}) {
  ...
  const deps = { clone: structuredClone, ownsPersonalMirror: () => false, readPersonalLocalValue: key => values.get(key) ?? null, state, ADMIN_TEMPLATE_PHOTO_APPEND_ENABLED: enabled, ADMIN_TEMPLATE_PHOTO_EDIT_ENABLED: editEnabled, localStorageScopeKey: "id:admin-a",
