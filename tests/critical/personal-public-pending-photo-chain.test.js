...
import { personalPublicPendingPhotoFormChain as chain } from "../../src/sync/personal-public-pending-photo-chain.js";
import { preparePersonalPendingPhotoForm } from "../../src/sync/personal-pending-photo-form-plan.js";
import { personalPublicPhotoResultReference } from "../../src/sync/personal-pending-public-update.js";
  ...
  return { version: 1, snapshot: structuredClone(payload), mergeBase: { payload: structuredClone(payloadOf(parent)), stateRevision: 7 },
    action: { ...root.action, operationId: randomUUID(), kind: "list.update", generation: parent.action.generation + 1,
      body: { baseStateRevision: 7, payload, ...(userDeletion ? { userDeletion } : {}),
        photoResults: source === root ? personalPublicPhotoResultReference(source) : personalFormPhotoResultReference(source), causal: causal(root, parent, source) } } };
}
function form(root, parent, binding, entityType, entityId) {
  const base = payloadOf(parent), plan = preparePersonalPendingPhotoForm({ binding, snapshot: base, basePayload: base,
    baseStateRevision: 7, parentOperationId: parent.action.operationId, publicOperationId: root.action.operationId,
...
async function fixture(fileless = false) {
  const f = await publicImportFixture(fileless, { containerPhotos: true });
...
for (const fileless of [false, true]) test(`public ${fileless ? "fileless" : "multi-owner photo"} root retains both later file forms through fields, deletion and another owner`, async () => {
  ...
  assert.equal(result.publicOperationId, f.root.action.operationId);
  assert.equal(f.firstEdit.action.body.photoResults.version, 7);
  assert.equal(f.removed.action.body.photoResults.version, 8);
  assert.equal(f.final.action.body.photoResults.version, 8);
...
test("public file ancestry rejects relabelled imports, skipped roots/parents, wrong owners and forged inventory", async () => {
  ...
  for (const mutate of [f => { f.records = f.records.filter(record => record !== f.root); },
    ...
    f => { f.firstForm.action.body.ownerResult.publicOperationId = randomUUID(); },
    ...
    f => { f.root.action.body.guestImport = f.root.action.body.publicImport; delete f.root.action.body.publicImport; },
...
test("public form DB descendants bind every pending owner and accept only exact published substitutions", async () => {
  ...
  const receipt = { ok: true, stateRevision: 15, list: { id: "list", stateRevision: 15, payload: published },
    publicPhotoFormSourceOperationId: source.action.operationId, publicPhotoForm: { version: 1,
      publicOperationId: f.root.action.operationId, pendingPhotos: personalPublicPendingPhotoInventory(changed, "list") } };
...
for (const contextChange of [null, "actorId", "generation"]) test(`real public outbox retains mixed binary records and stops a changed context (${contextChange || "same editor, OFF recovery"})`, async () => {
  const f = await publicImportFixture(false, { containerPhotos: true }), values = new Map(), native = new Map();
  ...
  const make = (enabled = true, overrides = {}) => createPersonalSaveOutbox({ ...f.binding, storage, photoEnabled: enabled, photoBatchEnabled: enabled,
    photoFormEnabled: enabled, photoEditEnabled: enabled, formOwnerResultEnabled: enabled, publicPhotoFormEnabled: enabled,
    publicImportEnabled: enabled, pendingPublicUpdateEnabled: enabled, pendingFormUpdateEnabled: enabled, ...overrides });
  const outbox = make(), current = { ...f.binding, scope: "personal", generation: "opened-public-form" }, getContext = () => current;
  ...
  const store = { binding: f.binding, ids: async () => [...native.keys()],
    read: id => { const row = native.get(id); return row ? (row.kind === "public" ? decodePersonalPublicImportRecord : decodePersonalPhotoFormRecord)(row.bytes, f.binding, id) : null; },
    capturePublic: async value => native.set(value.action.operationId, { kind: "public", bytes: await encodePersonalPublicImportRecord({ binding: f.binding, ...value }) }),
  ...
  const prepared = await preparePersonalPublicImport({ selection: f.selection, enabled: true, outbox, store, getContext,
    selectionStore: createPersonalPublicImportSelectionStore({ binding: f.binding, storage, locks: { request: async (_name, run) => run() }, getContext, enabled: true }),
  ...
  if (contextChange) {
    ...
    const session = createPersonalPendingPhotoFormSession({ outbox, store, getContext, enabled: true, publicEnabled: true,
  ...
  const submit = (entityType, entityId) => {
    ...
    return createPersonalPendingPhotoFormSession({ outbox, store, getContext, enabled: true, publicEnabled: true, onDurable: record => {
  ...
  assert.equal(edit.action.body.photoResults.version, 8);
  ...
  const oldGatesOnly = make(true, { publicPhotoFormEnabled: false, photoBatchCancellationEnabled: true });
  ...
  await assert.rejects(oldGatesOnly.drain({ queue, getContext, photoStore: store, photoStaging: staging }), { code: "public-photo-form-disabled" });
  await assert.rejects(oldGatesOnly.cancelPhotoUpload({ queue, getContext, photoStore: store, photoStaging: staging }), { code: "public-photo-form-disabled" });
  await assert.rejects(drainPersonalPhotoForm({ outbox: oldGatesOnly, store, staging, queue, getContext, enabled: true,
    publicEnabled: true, formOwnerResultEnabled: true, pendingFormUpdateEnabled: true, onAdopted: unexpected }), { code: "photo-form-drain" });
  assert.equal(personalPhotoRecoveryCancellationHead(last, { records: oldGatesOnly.list(), batchEnabled: true, formEnabled: true,
    editEnabled: true, formOwnerResultEnabled: true, publicEnabled: true, pendingFormUpdateEnabled: true }), false);
...
function networkFixture(record, overrides = {}) {
  ...
  const oldCapabilities = ["personalListCausalOperationsV1", "personalListOperationCancellationV1", "personalCausalPhotoPublicationV1",
    "personalCausalPhotoFormV1", "personalCausalPhotoFormOwnerResultV1", "personalCausalPhotoFormDescendantsV1", "personalCausalPublicImportV1",
  ...
  const fetchImpl = async (path, options) => {
    ...
    assert.equal(options.method, "GET", "unsupported public continuation never POSTs");
  ...
  const queue = createListOperationQueue({ transport, getContext, fetchImpl, locks, enabled: true, photoEnabled: true,
    photoFormEnabled: true, formOwnerResultEnabled: true, pendingFormUpdateEnabled: true, publicImportEnabled: true,
...
test("public continuation queue blocks both new forms and DB descendants with its own flag OFF", async () => {
  const f = await fixture();
  for (const record of [f.firstForm, f.final]) for (const option of ["publicPhotoFormEnabled", "publicImportEnabled", "formOwnerResultEnabled"]) {
    const n = networkFixture(record, { publicPhotoFormEnabled: true, [option]: false });
...
test("old server capabilities cannot claim public continuation forms, descendants or cancellation", async () => {
  const f = await fixture();
  for (const record of [f.firstForm, f.final]) for (const method of ["run", "cancelExact"]) {
    const n = networkFixture(record, { publicPhotoFormEnabled: true });
...
test("waiting public continuation rechecks server support and rejected receipts remain readable with writers OFF", async () => {
  const f = await fixture();
  for (const record of [f.firstForm, f.final]) {
    const n = networkFixture(record, { publicPhotoFormEnabled: true }), { action } = record;
...
test("a new public form's file cannot be staged or cancelled using only older photo gates", async () => {
  ...
  const make = overrides => createPersonalPhotoStaging({ store, transport: n.transport, getContext: n.getContext,
    ...
    formEnabled: true, publicEnabled: true, ...overrides });
  ...
  for (const method of ["stage", "cancel"]) {
    await assert.rejects(make({ publicPhotoFormEnabled: true })[method](action.operationId, stageId), { isPhotoStagingBlocked: true });
    assert.ok(n.calls.some(call => call.path === "/bike-packing/capabilities"));
  }
});
