...
export async function drainPersonalSaveWithReconciliation({ outbox, queue, getContext,
  readRemote, makeSnapshot, makeBaselineMeta, resolveConflicts, onReconciled, onAdopted, onConfirmed, maxReconciliations = 2 }) {
  for (let attempt = 0; ; attempt++) {
    try { return await outbox.drain({ queue, getContext, onConfirmed }); }
      ...
      const record = await outbox.reconcile({ queue, getContext, readRemote, makeSnapshot, makeBaselineMeta, resolveConflicts });
