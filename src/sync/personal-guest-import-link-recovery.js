...
// Explicit recovery of the native-file commit -> durable outbox-link gap.
...
export async function recoverPersonalGuestImportLink({ entry, outbox, store, getContext, enabled = PERSONAL_GUEST_IMPORT_ENABLED }) {
  ...
  if (!outbox.recover()) await outbox.adoptRemoteBaseline({ snapshot: entry.selection.basePayload, payload: entry.selection.basePayload,
    stateRevision: entry.selection.baseStateRevision }, { assertCurrent });
  assertCurrent();
