...
export function preparePersonalPhotoEditForm({ binding, snapshot, basePayload, baseStateRevision, entityType, entityId,
  fields, photoIds, baseEntityRevision, photoRevisions, operationId, formContext = null, containerFormContext = null }, {
  enabled = PERSONAL_PHOTO_EDIT_FORM_ENABLED, itemContextEnabled = PERSONAL_PHOTO_ITEM_FORM_CONTEXT_ENABLED,
  containerContextEnabled = PERSONAL_PHOTO_CONTAINER_FORM_CONTEXT_ENABLED, snapshotToPayload = value => value } = {}) {
  ...
  const input = { binding, snapshot, basePayload, baseStateRevision, entityType, entityId, fields, photoIds, baseEntityRevision, photoRevisions, operationId,
    ...(formContext === null ? {} : { formContext }), ...(containerFormContext === null ? {} : { containerFormContext }) };
  assertListOperationPayload({ ...binding, kind: "photos.mutate", body: input });
  ({ binding, snapshot, basePayload, fields, photoIds, photoRevisions, formContext = null, containerFormContext = null } = clone(input));
  ...
  const body = { ...common, action: "form", baseStateRevision, fields, changes, ...(formContext === null ? {} : { formContext }), ...(containerFormContext === null ? {} : { containerFormContext }) };
  ...
  if (containerFormContext !== null) {
    const context = applyPersonalPhotoContainerFormContext(snapshot, body, basePayload);
    refreshPersonalPhotoContainerContextView(snapshot, context);
  }
...
export function createPersonalPhotoEditFormSession({ outbox, store, getContext, readOwner, onDurable,
  enabled = PERSONAL_PHOTO_EDIT_FORM_ENABLED, itemContextEnabled = PERSONAL_PHOTO_ITEM_FORM_CONTEXT_ENABLED, containerContextEnabled = PERSONAL_PHOTO_CONTAINER_FORM_CONTEXT_ENABLED,
    ...
    submit(input) {
      ...
      try {
        ...
        attempt.preview = preparePersonalPhotoEditForm({ ...request, operationId: attempt.operationId, baseEntityRevision: 1,
          photoRevisions: photos.map(photo => ({ photoId: photo.id, assetId: photo.assetId, photoRevision: 1 })) }, { enabled, itemContextEnabled, containerContextEnabled, snapshotToPayload }).snapshot;
        current(initial);
        ...
        (async () => {
          ...
          const prepared = preparePersonalPhotoEditForm({ ...request, ...versions, operationId: attempt.operationId }, { enabled, itemContextEnabled, containerContextEnabled, snapshotToPayload });
