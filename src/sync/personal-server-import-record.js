...
import { personalServerImportManifest, assertPersonalServerImportHashes } from "./personal-server-import-protocol.js";
...
const invalid = () => { throw Object.assign(Error("Сохранённая копия списка по ссылке не совпадает с её файлами."), { code: "server-import-record" }); };
function validateIntent({ binding, action, snapshot, files }) {
  ...
  const manifest = personalServerImportManifest(action.body?.serverImport);
...
const verify = saved => {
  const manifest = personalServerImportManifest(saved.action.body.serverImport);
...
export async function encodePersonalServerImportRecord(input) {
  ...
  await decodePersonalServerImportRecord(record, binding, operationId); return record;
}
export async function decodePersonalServerImportRecord(record, binding, operationId) {
  const saved = await codec.decode(record, binding, operationId);
  await assertPersonalServerImportHashes(saved.action.body); return verify(saved);
}
