...
import { adminTemplatePhotoTreeCopyIntent, adminTemplatePhotoTreeCopyStageManifest,
  adminTemplatePhotoTreeCopyStageManifests, adminTemplatePhotoTreeCopyPayload } from "./admin-template-photo-tree-copy-protocol.js";
...
const mode = "admin-template-photo-tree-copy", maxReceiptBytes = 4 * 1024 * 1024;
...
function stageShape(data, manifest, assetDigest) {
  ...
  return exact(r, ["version", "kind", "manifest", "assetDigest", "sourceOwnerId", "ownerId", "baseEntityRevision", "sourceStored", "stored", "materialization"])
    && r.version === 2 && r.kind === mode && r.assetDigest === assetDigest && same(r.manifest, manifest)
...
export async function validateAdminTemplatePhotoTreeCopyStageReceipt(input, expected) {
  try {
    const data = clone(input), manifest = adminTemplatePhotoTreeCopyStageManifest(expected.manifest), assetDigest = expected.assetDigest;
...
async function expectedStages(intent) {
  const manifests = await adminTemplatePhotoTreeCopyStageManifests(intent), assets = flatAssets(intent);
  for (const [index, manifest] of manifests.entries()) if (await digest(manifest) !== assets[index].assetDigest) throw Error("tree-stage-binding");
...
export async function validateAdminTemplatePhotoTreeCopyStages(input, values) {
  try {
    const intent = adminTemplatePhotoTreeCopyIntent(input), stages = clone(values);
    return stagesMatch(stages, await expectedStages(intent));
  } catch { return false; }
}

function resultStructure(result, intent) {
  const c = intent.body.photoCopy, root = c.owners.find(owner => owner.sourceEntityId === c.source.rootId);
  return exact(result, ["version", "sourceOwnerId", "ownerId", "rootId", "owners", "confirmedPayload", "confirmedPayloadDigest"])
    && result.version === 2 && ownerId(result.sourceOwnerId) && ownerId(result.ownerId) && result.rootId === root.entityId
    && hash(result.confirmedPayloadDigest) && bytes(result).byteLength <= maxReceiptBytes
    && same(result.confirmedPayload, adminTemplatePhotoTreeCopyPayload(intent, result.owners));
}
export function validateAdminTemplatePhotoTreeCopyResultStructure(input, value) {
  try { return resultStructure(clone(input), adminTemplatePhotoTreeCopyIntent(value)); } catch { return false; }
}
...
export async function validateAdminTemplatePhotoTreeCopyResult(input, expected) {
  try {
    const result = clone(input), intent = adminTemplatePhotoTreeCopyIntent(expected.intent), stages = clone(expected.stageReceipts);
...
export async function validateAdminTemplatePhotoTreeCopyReceipt(input, expected) {
  try {
    const receipt = clone(input), intent = adminTemplatePhotoTreeCopyIntent(expected.intent), payloadDigest = expected.payloadDigest;
    ...
    if (receipt.operation.state !== "committed" || status !== 200
      ...
      || !positive(payload.stateRevision) || payload.stateRevision !== intent.body.base.stateRevision + 1
