...
import { encodeAdminTemplatePhotoCopyRecord, decodeAdminTemplatePhotoCopyRecord } from "./admin-template-photo-copy-record.js";

const databaseName = "bike-packing-admin-template-photo-copy-actions-v1";
...
const blocked = (code, cause) => Object.assign(Error("Сохранённое копирование требует сверки. Исходный выбор сохранён для восстановления."),
  { code: `admin-template-photo-copy-storage-${code}`, cause, isAdminTemplateBlocked: true });
...
export function createAdminTemplatePhotoCopyActionStore({ binding, getContext, indexedDB = globalThis.indexedDB,
  ...
  const decode = async (raw, operationId, initial) => {
    ...
    const value = await decodeAdminTemplatePhotoCopyRecord(raw, binding, operationId); guard(initial); return value;
  };
  ...
  return Object.freeze({ binding,
    async capture(input) {
      ...
      try {
        const record = await encodeAdminTemplatePhotoCopyRecord({ binding, ...frozen }); guard(initial);
        const prior = new Map();
        ...
        try { guard(initial); error.unconfirmedAdminPhotoCopy = frozen; } catch { /* The original actor retains any committed record. */ }
        throw error;
      }
    },
    ...
    async readStage(operationId, stageId) {
      ...
      return { binding: record.binding, action: record.action, snapshot: record.snapshot, intentHash: record.intentHash, stage,
        assetDigest: record.action.body.photoCopy.assets.find(value => value.assetId === stageId).assetDigest };
    },
    async claimStage(operationId, stageId) {
      ...
      const stage = record?.stages.find(value => value.operationId === stageId), asset = record?.action.body.photoCopy.assets.find(value => value.assetId === stageId);
