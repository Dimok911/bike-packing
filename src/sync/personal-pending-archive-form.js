...
const fail = () => { throw Error("Форма больше не совпадает с сохранённым архивом. Поля остались в открытой форме."); };

// Field-only continuation of a frozen archive. It never touches the archive,
...
export function createPersonalPendingArchiveFormSession({ outbox, getContext, onDurable,
  snapshotToPayload = value => value, createUuid = () => crypto.randomUUID(), enabled = PERSONAL_PENDING_ARCHIVE_UPDATE_ENABLED }) {
  ...
  return {
    submit(input) {
      ...
      try {
        if (!enabled || !outbox || typeof onDurable !== "function") fail();
        assertListOperationPayload({ ...outbox.binding, kind: "list.update", body: input });
        const frozen = clone(input); initial = clone(getContext());
        const head = outbox.recover(), source = personalPendingArchiveUpdateSource({ records: outbox.list(),
    ...
    recoveryCopy() {
      ...
      return preview && initial?.scope === "personal" && context?.scope === "personal"
        ...
        ? { version: 1, phase: "pending-archive-fields", operationId, request: clone(request), preview: clone(preview), files: [], automaticImportAllowed: false } : null;
    }
  };
}
