...
import { personalPublicImportManifest, assertPersonalPublicImportHashes } from "./personal-public-import-protocol.js";
...
const invalid = () => { throw Object.assign(Error("Сохранённый копирование шаблона и его файлы не совпадают."), { code: "public-import-record" }); };
function validateIntent({ binding, action, snapshot, files }) {
  ...
  const manifest = personalPublicImportManifest(action.body?.publicImport);
...
const verify = saved => {
  const manifest = personalPublicImportManifest(saved.action.body.publicImport);
...
export async function encodePersonalPublicImportRecord(input) {
  ...
  await decodePersonalPublicImportRecord(record, binding, operationId); return record;
}
export async function decodePersonalPublicImportRecord(record, binding, operationId) {
  const saved = await codec.decode(record, binding, operationId);
  await assertPersonalPublicImportHashes(saved.action.body); return verify(saved);
}
