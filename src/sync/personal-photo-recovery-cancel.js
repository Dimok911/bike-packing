...
export const personalPhotoRecoveryCancellationHead = (record, { batchEnabled = PERSONAL_PHOTO_BATCH_CANCELLATION_ENABLED,
  ...
  if (([6, 7].includes(record?.action?.body?.ownerResult?.version) || [13, 14].includes(record?.action?.body?.photoResults?.version))
    && (!serverPhotoFormEnabled || !serverEnabled)) return false;
  if (([2, 5].includes(record?.action?.body?.ownerResult?.version) || [8, 11].includes(record?.action?.body?.photoResults?.version))
    && (!publicPhotoFormEnabled || !publicEnabled)) return false;
  if (([3, 4].includes(record?.action?.body?.ownerResult?.version) || [9, 10].includes(record?.action?.body?.photoResults?.version))
    && !importPhotoFormEnabled) return false;
  if ((record?.action?.body?.ownerResult || [6, 8, 9, 10, 11, 13, 14].includes(record?.action?.body?.photoResults?.version))
  ...
  const importChain = personalPendingPhotoFormChain({ records, operationId: record?.action?.operationId, listId: record?.action?.listId });
  if (importChain?.serverOperationId && importChain.forms.length > 1 && (!serverPhotoFormEnabled || !serverEnabled)) return false;
  if (!serverNewOwnerFormEnabled && (record?.action?.body?.ownerResult?.version === 7 || record?.action?.body?.photoResults?.version === 14
    || importChain?.forms.some(form => form.action.body.ownerResult?.version === 7))) return false;
  if (!publicNewOwnerFormEnabled && (record?.action?.body?.ownerResult?.version === 5 || record?.action?.body?.photoResults?.version === 11
    || importChain?.forms.some(form => form.action.body.ownerResult?.version === 5))) return false;
  if (!importNewOwnerFormEnabled && (record?.action?.body?.ownerResult?.version === 4 || record?.action?.body?.photoResults?.version === 10
    || importChain?.forms.some(form => form.action.body.ownerResult?.version === 4))) return false;
  if (importChain?.importOperationId && importChain.forms.length > 1 && (!importPhotoFormEnabled
    || (importChain.importKind === "guest" ? !guestEnabled : !archiveEnabled))) return false;
  return (
  ...
  || record?.action?.kind === "list.import" && (serverEnabled && [1, 2].includes(record.action.body.serverImport?.version) || publicEnabled && (record.action.body.publicImport?.version !== 2 || publicEntityEnabled) && [1, 2].includes(record.action.body.publicImport?.version) || guestEnabled && record.action.body.guestImport?.version === 1
  ...
  || (archiveEnabled || guestEnabled || publicEnabled || serverEnabled) && record?.reconciliation?.decision?.type === "keep-server-after-rejected-import" && validPersonalRestoreCancellation(record)
  ...
  || serverEnabled && batchEnabled && Boolean(personalPendingServerUpdateSource({ records, operationId: record?.action.operationId, listId: record?.action.listId }))
...
export async function cancelPersonalPhotoRecovery({ outbox, store, transport, getContext, readRemote, makeSnapshot, makeBaselineMeta,
  ...
  return locks.request(`bike-packing-photo-recovery-decision:${JSON.stringify(binding)}`, async () => {
    ...
    const pendingForm = pendingFormUpdateEnabled && pendingFormForCancellation(head, outbox.list(), formEnabled, editEnabled, manufacturerSourceEnabled)
      ...
      || serverEnabled && personalPendingServerUpdateSource({ records: outbox.list(), operationId: head?.action.operationId, listId: binding.listId })
    ...
    if (!personalPhotoRecoveryCancellationHead(head, { batchEnabled, formEnabled, editEnabled, copyEnabled, copyBatchEnabled, archiveEnabled, guestEnabled, publicEnabled, publicEntityEnabled, serverEnabled, pendingArchiveUpdateEnabled, pendingGuestUpdateEnabled, pendingPublicUpdateEnabled, pendingFormUpdateEnabled, pendingOwnerDeletionEnabled, pendingCopyDeletionEnabled, pendingCopyBatchDeletionEnabled,
      manufacturerSourceEnabled, formOwnerResultEnabled, publicPhotoFormEnabled, publicNewOwnerFormEnabled, importPhotoFormEnabled, importNewOwnerFormEnabled, serverPhotoFormEnabled, serverNewOwnerFormEnabled, records: outbox.list() })) throw Error("Это составное действие требует отдельного восстановления. Исходные данные сохранены.");
    const queue = createListOperationQueue({ transport, getContext, fetchImpl, locks, enabled: true, photoEnabled: true,
      ...
      guestImportEnabled: guestEnabled, publicImportEnabled: publicEnabled, publicEntityEnabled, serverImportEnabled: serverEnabled,
      pendingArchiveUpdateEnabled, pendingGuestUpdateEnabled, pendingPublicUpdateEnabled, pendingFormUpdateEnabled, manufacturerSourceEnabled, formOwnerResultEnabled, publicPhotoFormEnabled, publicNewOwnerFormEnabled, importPhotoFormEnabled, importNewOwnerFormEnabled, serverPhotoFormEnabled, serverNewOwnerFormEnabled,
      cancellationEnabled: operationCancellationEnabled });
    const photoStaging = createPersonalPhotoStaging({ store, transport, getContext, fetchImpl, locks, enabled: true, cancellationEnabled: true, batchEnabled, formEnabled, archiveEnabled, guestEnabled, publicEnabled, publicEntityEnabled, serverEnabled, publicPhotoFormEnabled, publicNewOwnerFormEnabled, importPhotoFormEnabled, importNewOwnerFormEnabled, serverPhotoFormEnabled, serverNewOwnerFormEnabled });
    ...
    try {
      ...
      if (!(validPersonalPhotoCancellation(head) || (archiveEnabled || guestEnabled || publicEnabled || serverEnabled) && validPersonalRestoreCancellation(head)) || !error.isOperationReceiptError || error.isPersonalSaveBlocked) throw error;
    }
    assertContext();
