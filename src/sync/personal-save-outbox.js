...
export function createPersonalSaveOutbox({ storage, actorId, listId, scopeKey,
    ...
    try {
      ...
      for (const record of records.values()) {
        ...
        if (Object.hasOwn(action.body, "photoResults")) {
          const form = [5, 6, 8].includes(action.body.photoResults.version) ? personalPendingFormUpdateSource({ records: [...records.values()], operationId: action.operationId, listId })
  ...
  return {
    ...
    preparePhoto({ snapshot, payload, body, operationId = crypto.randomUUID() }) {
      ...
      if (dependent) {
        ...
        if (!chain || applied.has(head.action.operationId) || input.body.ownerResult.operationId !== head.action.operationId
          || (chain.publicOperationId ? input.body.ownerResult.version !== 2 || input.body.ownerResult.publicOperationId !== chain.publicOperationId : input.body.ownerResult.version !== 1)
    ...
    capture({ snapshot, body, create = false, restore = false, migration = false, archiveImport = false, operationId = crypto.randomUUID(), localReconciliation = null }) {
      ...
      const pendingForm = formSource && (pendingFormUpdateEnabled || [5, 6, 8].includes(head.action.body.photoResults?.version)) ? formSource : null;
      if (pendingForm) {
    ...
    async cancelPhotoUpload({ queue, getContext, photoStore, photoStaging }) {
      ...
      if ((head?.action.body.ownerResult || [6, 8].includes(head?.action.body.photoResults?.version)) && !formOwnerResultEnabled) throw blocked("photo-cancellation", "Отмена связанных фотоформ ещё не включена.");
