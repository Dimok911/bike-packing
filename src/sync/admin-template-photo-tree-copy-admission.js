...
export function createAdminTemplatePhotoTreeCopyAdmission({ binding, store, getContext,
  ...
  const load = async (id, check) => {
    ...
    const expected = await prepareAdminTemplatePhotoTreeCopyRecord({ binding, action: record.action, snapshot: record.snapshot }); check();
    ...
    try {
      await enter(proof, async scope => {
        ...
        const assertCurrent = () => {
          ...
          check(); synchronous(scopeGuard); check();
        };
  ...
  return Object.freeze({ binding,
    async run(operationId, task) {
      ...
      try {
        const first = await load(operationId, baseGuard), source = first.action.body.photoCopy.source;
        ...
        return await withAdminTemplateCapture({ bindings, locks }, async captureLease => {
          ...
          const validate = (check, work) => scoped("admin-template-photo-tree-copy-inventory-v1", withInventory, proof, check,
            inventoryGuard => scoped("admin-template-photo-tree-copy-namespaces-v1", withNamespaces, proof, inventoryGuard, work));
          return await validate(guard, async assertCurrent => {
            ...
            const withDispatchAdmission = async (requested, work) => {
              ...
              const check = () => { assertCurrent(); synchronous(requestedGuard); assertCurrent(); };
              try {
