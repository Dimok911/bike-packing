...
export async function prepareAdminTemplateTreeCopy(state, request, { operationId, changedAt = "", currentEditMeta = () => ({}),
  ...
  const frozen = clone(state), { rootId, sourceLayoutId, targetLayoutId, includeContents, targetParentId = "", targetIndex = null } = clone(request);
  const layout = frozen.layouts?.[targetLayoutId], arrangement = layout?.arrangement;
  if (!validTemplateOperationId(operationId) || typeof includeContents !== "boolean" || sourceLayoutId !== targetLayoutId
  ...
  const entries = ["containers", "items"].flatMap(type => Object.keys(source[type]).sort().map((sourceId, index) => ({
    type, sourceId, targetId: `${type === "items" ? "item" : "container"}-template-tree-${operationId}-${index}`
  })));
  if (entries.some(entry => ["layouts", "containers", "items"].some(type => Object.hasOwn(frozen[type] || {}, entry.targetId)))) fail();
  const snapshot = clone(frozen);
  const copied = await duplicateContainerSnapshotRecords(source, { targetState: snapshot, changedAt, currentEditMeta, normalizeContainerColor,
  ...
  for (const entry of entries) {
    if (entry.type === "items") {
      ...
      snapshot.items[entry.targetId].quantity = 1;
  ...
  return { snapshot, entries, rootId: copied.rootId, source };
}
