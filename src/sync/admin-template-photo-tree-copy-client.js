...
import { ADMIN_TEMPLATE_PHOTO_TREE_COPY_ENABLED, TEMPLATE_PHOTO_TREE_COPY_CAPABILITY,
  adminTemplatePhotoTreeCopyIntent } from "./admin-template-photo-tree-copy-protocol.js";
...
const stagePath = "/bike-packing/admin/template-photo-assets/tree-copy", copyPath = stagePath, commandPath = "/bike-packing/admin/template-operations";
const kind = "admin-template-photo-tree-copy", clone = value => JSON.parse(canonical(value)), same = (a, b) => canonical(a) === canonical(b);
...
const blocked = (code = "paused", cause) => Object.assign(Error("Копирование сохранено и требует сверки. Продолжите исходное действие."),
  { code: `admin-template-photo-tree-copy-client-${code}`, cause, isAdminTemplateBlocked: true });
...
export function createAdminTemplatePhotoTreeCopyClient({ binding, getContext, store, transport, storage = globalThis.localStorage,
  ...
  const loadRecord = async (id, guard) => {
    ...
    const record = clone(raw), expected = await prepareAdminTemplatePhotoTreeCopyRecord({ binding, action: record.action, snapshot: record.snapshot }); guard();
  ...
  const read = async (id, guard) => {
    ...
    for (const [index, receipt] of saved.stageReceipts.entries()) if (receipt !== null) {
      if (!await validateAdminTemplatePhotoTreeCopyStageReceipt(receipt, { manifest: record.stages[index], assetDigest: assets(saved.intent)[index].assetDigest })) throw blocked("stage-receipt");
      guard();
    }
    if (saved.dispatched && saved.stageReceipts.some(value => value === null)) throw blocked("journal");
    if (saved.stageReceipts.every(value => value !== null) && !await validateAdminTemplatePhotoTreeCopyStages(saved.intent, saved.stageReceipts)) throw blocked("stages");
    guard();
    if (saved.receipt !== null && !await validateAdminTemplatePhotoTreeCopyReceipt(saved.receipt, { intent: saved.intent,
  ...
  const stageMetadata = (saved, index) => ({ type: "admin-template-photo-stage", protocol: "admin-template-photo-tree-copy-stage-v2", ...binding,
        ...
        return await withLock(id, async () => {
          ...
          const capabilities = async () => {
            ...
            if (value.service !== "bikepacking-api" || !Array.isArray(value.capabilities)
              || ![TEMPLATE_OPERATION_CAPABILITY, TEMPLATE_PHOTO_APPEND_CAPABILITY, TEMPLATE_PHOTO_CREATE_CAPABILITY, TEMPLATE_PHOTO_COPY_CAPABILITY, TEMPLATE_PHOTO_TREE_COPY_CAPABILITY]
          ...
          const settleStage = async (index, data) => {
            ...
            if (!await validateAdminTemplatePhotoTreeCopyStageReceipt(data, { manifest: record.stages[index], assetDigest: asset.assetDigest })) throw blocked("stage-receipt");
          ...
          if (!await validateAdminTemplatePhotoTreeCopyStages(saved.intent, saved.stageReceipts)) throw blocked("stages");
          guard();
          return admitted(async () => {
  ...
  return Object.freeze({
    ...
    async capture(input) {
      ...
      const intent = adminTemplatePhotoTreeCopyIntent({ ...binding, ...action });
