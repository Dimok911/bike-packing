...
export function createPersonalOrdinaryRecoveryStore({ storage, binding: rawBinding }) {
  ...
  const publish = (key, value) => {
    ...
    try { if (storage.getItem(key) !== null) fail(); storage.setItem(key, raw); if (storage.getItem(key) !== raw) fail(); }
    catch { fail(); }
    return raw;
  };
  return {
    ...
    prepare({ entries, snapshot, operationIds, headOperationId, headGeneration }) {
      ...
      parseArchive(JSON.stringify(archive), key); const raw = publish(key, archive);
    ...
    complete(archive, recordRaw) {
      ...
      publish(`${keyPrefix}complete:${archive.recoveryId}`, { version: 1, recoveryId: archive.recoveryId, recordRaw });
