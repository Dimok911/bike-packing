...
import { assertPersonalArchivePhotoBody } from "./personal-archive-photo-protocol.js";

const same = (a, b) => personalArchiveJson(a) === personalArchiveJson(b);
const fail = () => { throw Object.assign(Error("Архив, очередь и полный набор файлов не совпадают. Отправка остановлена."), { code: "archive-photo-record", isPersonalSaveBlocked: true }); };
export function assertPersonalArchivePhotoRecord(record) {
  const action = record?.action, photo = record?.photoState;
  if (action?.kind !== "list.import" || action.body?.archiveImport?.version !== 2 || photo?.version !== 1
    ...
    || !same(photo.payload, action.body.payload) || !same(personalArchivePayloadWithPhotos(record.snapshot), photo.payload)) fail();
  const plan = assertPersonalArchivePhotoBody(action.body, { base: record.mergeBase.payload, listId: action.listId, causal: true });
...
export function assertPersonalArchivePhotoFile(record, saved, binding) {
  const attachments = assertPersonalArchivePhotoRecord(record);
