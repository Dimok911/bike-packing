export function createAppTailControllers(ctx) {
  ...
  const {
    ...
    activeReadOnlyLayoutId, activeReadOnlyLayoutIdFromScope, addBackupDictionaryValues, addCustomDictionaryValue, addItemToLayoutArrangementForState,
    ...
    bindFixedScrollbar, bindHorizontalTouchScroll, bindLayoutEditorControls, bindPackingEventsUi, bindPhotoGalleries,
    ...
    defaultDemoState, defaultRootContainerLocation, deleteCachedPhoto, deleteItemFromState, deletePublishedSharedTemplateRecord,
    ...
    dictionaryOptionsForUiValues, dictionarySelectEntry, dictionarySortModeForType, dictionaryValueLabel, duplicateContainerSnapshotRecords,
    ...
    moveContainerInLayoutArrangementForState, moveItemInLayoutArrangementForState, moveRootColumnInState, nextDemoTemplateAfter, nextItemDisplayModeValue,
    ...
    renderListItemHtml, renderPackingItemCardHtml, renderPhotoGalleryHtml, renderPreservingPackingScroll, renderRootContainerCardHtml,
    ...
    shouldClearPackingListContextForPrivateMutation, shouldCopyPublicTemplatePhotoReferencesOnServer, shouldCreatePublishedTemplateBeforePhotos, shouldDeletePublishedTemplateForLayoutValue, shouldImportGuestLayoutBeforeRemote,
    ...
    updateViewScopedControlsUi, uploadEntityPhoto, uploadEntityPhotoToPath, uploadPendingPhotos, uploadPublishedLayoutPhotos,
...
async function createAndPublishTemplateCopy(sourceLayout, requestedName) {
  ...
  try {
    ...
    assertPublishedTemplateCopyConfirmed(state.layouts?.[createdId], {
      serverConfirmedDemoTemplates: runtime.serverConfirmedDemoTemplates,
      serverConfirmedSharedLayouts: runtime.serverConfirmedSharedLayouts
    });
...
function cleanupUnsavedItemDialogPhotoDraft() {
  if (!runtime.itemDialogPhotoDraft || !runtime.editingItemId) return;
  const sourcePhotoIds = photoIdentitySet(normalizeItemPhotos(state.items?.[runtime.editingItemId] || { photos: [] }));
  runtime.itemDialogPhotoDraft.photos.forEach((photo) => {
...
function updateItemDialogPhotoPrimaryButton() {
  updatePhotoPrimaryButton(refs.itemPhotoPrimaryBtn, runtime.itemDialogPhotoActiveIndex);
}
...
function cleanupUnsavedRootContainerDialogPhotoDraft() {
  if (!runtime.rootContainerDialogPhotoDraft || !runtime.editingRootContainerId) return;
  const sourcePhotoIds = photoIdentitySet(normalizeItemPhotos(state.containers?.[runtime.editingRootContainerId] || { photos: [] }));
  runtime.rootContainerDialogPhotoDraft.photos.forEach((photo) => {
    if (photoIdentityMatches(sourcePhotoIds, photo)) return;
...
function updateRootContainerDialogPhotoPrimaryButton() {
  updatePhotoPrimaryButton(refs.rootContainerPhotoPrimaryBtn, runtime.rootContainerDialogPhotoActiveIndex);
}
