...
export function createAppTailControllers(ctx) {
  ...
  const {
    ...
    confirmCreateLayoutFromReadonlyTemplate, confirmLoadedDemoPublicTemplate, confirmPublicCopyDuplicates, confirmPublicLayoutTransition,
    confirmRepeatedSharedLayoutCopy, conflictDefaultChoice, conflictFormatter, conflictKindLabel, conflictLabel,
    conflictSummary, conflictTimestamp, conflictVersionStamp, containerCategories,
    ...
    importDemoStateAsEditableLayoutValue, init, initialRemoteLoadPending,
    ...
    normalizeUiLanguage, nowIso, offerLoadServerForTruncatedLocalState,
    offlineRememberedUser, openAdminDemoLayout, openAuthDialog, openCategoryFilterDialog,
    ...
    sharedPayloadActiveLayout, sharedRootFromPublishedContainer, shouldBlockLegacyPersonalSyncWrite, shouldBlockLegacyPersonalSyncWriteFallback,
    shouldAutoPublishManagedTemplate, shouldClearPackingListContextForPrivateMutation, shouldConfirmManagedTemplateTransition,
    shouldCopyPublicTemplatePhotoReferencesOnServer, shouldCreatePublishedTemplateBeforePhotos, shouldDeletePublishedTemplateForLayoutValue,
    ...
    stateIntegrityMetaFromResponse, statePrivateLayoutCount, stateStats, stateStatsForDestructiveComparison,
    storedPrivateLayoutChoiceRef, stripPublicOriginForPrivateCopy, stripPublishedPublicOriginMarkers, subcontainerTitleHtml,
  ...
  function openCatalogStockLocations(id) {
    ...
    stockLocationsDialog.open(state.items[id], (rows) => {
      if (setItemStockLocations(state.items[id], rows)) return saveInventoryChange(id);
    });
  }
  const preparationDialog = createPreparationDialogController({
    ...
    purchase: (id, value, layoutId, location) => {
      ...
      if (addPurchasedStock(state.items[id], value, location)) return saveInventoryChange(id);
      return false;
    },
    t
  });
  ...
  function saveInventoryChange(id) {
    ...
    touchItem(id, nowIso());
    return finishPreparedPersonalMutation(() => saveState(), () => { render(); return true; });
  }
...
async function applyManufacturerBagCatalogSelection(entry) {
  ...
  loadNoteFields(refs.rootContainerNote, draft);
...
function getContainerPickerLayoutOptions() {
  ...
  const newItemPlacementMode = isNewItemPlacementPickerMode(runtime.containerPickerMode);
  if (!copyMode && newItemPlacementMode && !runtime.editingItemId && adminTemplatePhotoCreateFormEnabled()) {
    const layout = state.layouts[getPublishedEditLayoutId()], source = layout?.adminCausalSource;
    return source?.exists && source.visibility === "private" && !source.deleted && !source.planId && !layout.templateDraftSyncPending
      && !layout.adminCausalCopyPlan ? [layout] : [];
  }
  ...
  if (comparison) {
    ...
    refs.packingView.innerHTML = `
      ...
      ${renderLayoutComparisonItemChangesHtml({
        comparison,
        escapeHtml,
        renderThumbnail: (record) => pickerListThumbnailHtml(record, { enabled: true, photoObjectUrls }),
        state,
        t
      })}
      ${renderLayoutComparisonBoardHtml({
...
function togglePacked(itemId) {
  ...
  if (adminTemplatePhotoCreateFormEnabled() && state.layouts[state.activeLayoutId]?.adminCausalSource?.photoOwnerMap) {
    state.layouts[state.activeLayoutId].arrangement.packedItems = clone(state.packedItems);
  }
...
function unpackAllItems() {
  ...
  openConfirmDialog({
    ...
    onConfirm: () => {
      ...
      state.packedItems = {};
      if (adminTemplatePhotoCreateFormEnabled() && state.layouts[state.activeLayoutId]?.adminCausalSource?.photoOwnerMap) {
        state.layouts[state.activeLayoutId].arrangement.packedItems = {};
      }
      state.showOnlyUnpacked = false;
      saveState();
      render();
...
function newEntityFormDraftStorageKey(kind) {
  return scopedLocalStorageKey(entityFormDraftStorageKey(kind));
}

function loadNewEntityFormDraft(kind) {
  if (adminTemplatePhotoCreateFormEnabled()) return null;
  const key = newEntityFormDraftStorageKey(kind);
  try {
    const raw = localStorage.getItem(key);
    if (!raw) return null;
    const draft = parseNewEntityFormDraft(raw, { kind });
...
function clearNewEntityFormDraftTimer(kind) {
  ...
  if (kind === "container") rootContainerFormDraftSaveTimer = null;
  else itemFormDraftSaveTimer = null;
}

function clearStoredNewEntityFormDraft(kind) {
  if (adminTemplatePhotoCreateFormEnabled() || adminTemplatePhotoCreateForms.owns(kind)) return;
  clearNewEntityFormDraftTimer(kind);
  try {
    localStorage.removeItem(newEntityFormDraftStorageKey(kind));
  } catch {
...
function newItemFormDraftFields() {
  return {
...
function newRootContainerFormDraftFields() {
  return {
    name: refs.rootContainerName?.value || "",
    weight: refs.rootContainerWeight?.value || "",
    volume: refs.rootContainerVolume?.value || "",
    color: refs.rootContainerColor?.value || "",
    width: refs.rootContainerWidth?.value || "",
    height: refs.rootContainerHeight?.value || "",
    depth: refs.rootContainerDepth?.value || "",
...
function persistNewItemFormDraft() {
  if (adminTemplatePhotoCreateFormEnabled() || adminTemplatePhotoCreateForms.owns("item")) return { meaningful: false, saved: false };
  clearNewEntityFormDraftTimer("item");
  if (!refs.dialog?.open || runtime.editingItemId || refs.saveItemBtn?.hidden) {
    return { meaningful: false, saved: false };
  }
  const snapshot = getItemDialogSnapshot();
  const meaningful = Boolean(runtime.itemDialogInitialSnapshot) &&
    !snapshotsEqual(snapshot, runtime.itemDialogInitialSnapshot);
  if (!meaningful) {
    clearStoredNewEntityFormDraft("item");
    return { meaningful: false, saved: true };
  }
  const draft = createNewEntityFormDraft({
    kind: "item",
    updatedAt: nowIso(),
    fields: newItemFormDraftFields(),
    context: {
      targetLayoutId: runtime.itemDialogTargetLayoutId || ""
    },
    photos: runtime.itemDialogPhotoDraft?.photos || [],
    photoUploadEntityId: photoDraftEntityId(runtime.itemDialogPhotoDraft)
  });
  const saved = Boolean(draft) && safeSetLocalStorage(
    newEntityFormDraftStorageKey("item"),
...
function persistNewRootContainerFormDraft() {
  if (adminTemplatePhotoCreateFormEnabled() || adminTemplatePhotoCreateForms.owns("container")) return { meaningful: false, saved: false };
  clearNewEntityFormDraftTimer("container");
  if (!refs.rootContainerDialog?.open || runtime.editingRootContainerId || refs.saveRootContainerBtn?.hidden) {
    return { meaningful: false, saved: false };
  }
  const snapshot = getRootContainerDialogSnapshot();
  const meaningful = Boolean(runtime.rootContainerDialogInitialSnapshot) &&
    !snapshotsEqual(snapshot, runtime.rootContainerDialogInitialSnapshot);
  if (!meaningful) {
  ...
  const draft = createNewEntityFormDraft({
    kind: "container",
    updatedAt: nowIso(),
    fields: newRootContainerFormDraftFields(),
    context: {
      placeInCurrentLayout: placeNewRootInCurrentLayout,
      targetLayoutId: rootContainerPlacementTargetLayoutId || ""
    },
    photos: runtime.rootContainerDialogPhotoDraft?.photos || [],
    photoUploadEntityId: photoDraftEntityId(runtime.rootContainerDialogPhotoDraft)
  });
  const saved = Boolean(draft) && safeSetLocalStorage(
    newEntityFormDraftStorageKey("container"),
    JSON.stringify(draft),
    { silent: true }
  );
  if (saved) syncNewEntityFormDraftCatalogCards();
...
function scheduleNewItemFormDraftSave() {
  if (!refs.dialog?.open || runtime.editingItemId || refs.saveItemBtn?.hidden) return;
  clearNewEntityFormDraftTimer("item");
  itemFormDraftSaveTimer = window.setTimeout(persistNewItemFormDraft, 250);
}

function scheduleNewRootContainerFormDraftSave() {
  if (!refs.rootContainerDialog?.open || runtime.editingRootContainerId || refs.saveRootContainerBtn?.hidden) return;
  clearNewEntityFormDraftTimer("container");
  rootContainerFormDraftSaveTimer = window.setTimeout(persistNewRootContainerFormDraft, 250);
}

function restoredNewEntityFormDraftPhotos(draft) {
  const record = { photos: Array.isArray(draft?.photos) ? draft.photos.map((photo) => ({ ...photo })) : [] };
  const photos = normalizeItemPhotos(record).map((photo) => ({
    ...photo,
    status: photo.status === "uploading" && !photo.url && !photo.thumbUrl ? "pending" : photo.status
  }));
  if (!photos.length) return null;
  return {
    photos,
    deletedPhotos: [],
    ...(draft.photoUploadEntityId ? { uploadEntityId: draft.photoUploadEntityId } : {})
  };
}

function restoreNewItemFormDraft() {
  const draft = loadNewEntityFormDraft("item");
  if (!draft) return false;
  const fields = draft.fields || {};
  const storedLayoutId = String(draft.context?.targetLayoutId || "");
  if (storedLayoutId && state.layouts?.[storedLayoutId]) runtime.itemDialogTargetLayoutId = storedLayoutId;
  ...
  if (refs.itemDepth) refs.itemDepth.value = String(fields.depth || "");
  ...
  const containerId = String(fields.containerId || "");
  const targetLayout = state.layouts?.[runtime.itemDialogTargetLayoutId];
...
function restoreNewRootContainerFormDraft() {
  const draft = loadNewEntityFormDraft("container");
  if (!draft) return false;
  const fields = draft.fields || {};
  const storedLayoutId = String(draft.context?.targetLayoutId || "");
  placeNewRootInCurrentLayout = Boolean(draft.context?.placeInCurrentLayout);
  rootContainerPlacementTargetLayoutId = placeNewRootInCurrentLayout && state.layouts?.[storedLayoutId]
    ? storedLayoutId
    : "";
  refs.rootContainerName.value = String(fields.name || "");
  refs.rootContainerWeight.value = String(fields.weight || "");
  refs.rootContainerVolume.value = String(fields.volume || "");
  if (refs.rootContainerColor) refs.rootContainerColor.value = String(fields.color || "");
  if (refs.rootContainerWidth) refs.rootContainerWidth.value = String(fields.width || "");
  if (refs.rootContainerHeight) refs.rootContainerHeight.value = String(fields.height || "");
  if (refs.rootContainerDepth) refs.rootContainerDepth.value = String(fields.depth || "");
  const location = String(fields.location || "");
...
function flushOpenEntityFormDrafts() {
  if (refs.dialog?.open && !runtime.editingItemId) persistNewItemFormDraft();
...
function openRootContainerDialog(containerId = null, {
  ...
  if (!containerId) restoreNewRootContainerFormDraft();
  updateRootContainerDialogSaveState();
...
function openItemDialog(itemId = null, { targetContainerId = "", targetLayoutId = "" } = {}) {
  ...
  if (!itemId) restoreNewItemFormDraft();
  updateItemDialogSaveState();
...
function updateLayoutOrderSaveState() {
  ...
  else refs.saveLayoutOrderBtn.removeAttribute("aria-busy");
  if (saving) {
    refs.saveLayoutOrderBtn.disabled = true;
    refs.saveLayoutOrderBtn.classList.remove("muted-save");
    refs.saveLayoutOrderBtn.textContent = t("layoutOrder.saving");
    refs.saveLayoutOrderBtn.removeAttribute("title");
...
function hasSavableLayoutOrderChanges() {
  if (layoutOrderSavePromise) return false;
  updateLayoutOrderSaveState();
...
async function requestCloseLayoutOrderDialog() {
  if (layoutOrderSavePromise) {
    await layoutOrderSavePromise;
    return;
  }
...
async function saveLayoutOrder(event) {
  event?.preventDefault();
  if (layoutOrderSavePromise) return layoutOrderSavePromise;
  if (!hasSavableLayoutOrderChanges()) return false;
  const sections = currentLayoutOrderSections();
  const savePromise = (async () => {
    try {
      await applyLayoutOrderSections(sections);
      if (refs.layoutOrderDialog?.open) refs.layoutOrderDialog.close("default");
      return true;
    } catch (error) {
      showToast(localText(
      ...
      if (layoutOrderSavePromise === savePromise) layoutOrderSavePromise = null;
      updateLayoutOrderSaveState();
    }
  })();
  layoutOrderSavePromise = savePromise;
  updateLayoutOrderSaveState();
  return savePromise;
}
...
const adminTemplatePhotoFormCallbacks = {
...
const adminTemplatePhotoCreateForms = createAdminTemplatePhotoCreateFormController({
  isEnabled: adminTemplatePhotoCreateFormEnabled,
  getContext: adminTemplatePhotoCreateFormContext,
  getView(type) {
    const item = type === "item";
    return { entityId: item ? runtime.editingItemId : runtime.editingRootContainerId, layoutId: getPublishedEditLayoutId(),
      token: item ? runtime.itemDialogInitialSnapshot : runtime.rootContainerDialogInitialSnapshot,
      dialog: item ? refs.dialog : refs.rootContainerDialog, saveButton: item ? refs.saveItemBtn : refs.saveRootContainerBtn,
      signature: JSON.stringify({ fields: item ? getItemDialogSnapshot() : getRootContainerDialogSnapshot(),
        targetLayoutId: item ? runtime.itemDialogTargetLayoutId : rootContainerPlacementTargetLayoutId,
        placeNewRootInCurrentLayout, pendingCopyTargetContainerSetup: Boolean(pendingCopyTargetContainerSetup),
        pendingParentId: runtime.rootContainerDialogPendingParentId, pendingParentIndex: runtime.rootContainerDialogPendingParentIndex,
        pendingRootIds: runtime.rootContainerDialogPendingRootIds,
        catalogSource: item ? runtime.sharedDialogCopyItemId : [rootContainerCatalogSelection, rootContainerManufacturerPhotoSource] }),
      draft: item ? runtime.itemDialogPhotoDraft : runtime.rootContainerDialogPhotoDraft };
  },
  readForm(type) {
    const item = type === "item", snapshot = item ? getItemDialogSnapshot() : getRootContainerDialogSnapshot();
    ...
    const layoutId = getPublishedEditLayoutId();
    const placement = item ? snapshot.containerId ? { layoutId: runtime.itemDialogTargetLayoutId || layoutId,
      containerId: snapshot.containerId, quantity: snapshot.quantity } : null
      : placeNewRootInCurrentLayout ? { layoutId: rootContainerPlacementTargetLayoutId || layoutId } : null;
    return { created: !(item ? runtime.editingItemId : runtime.editingRootContainerId),
      fields: { name: snapshot.name, weight: snapshot.weight, color: snapshot.color, location: snapshot.location,
        category: categories[0] || "", categories, note: snapshot.note, noteHtml: snapshot.noteHtml || "", dimensions: hasContainerDimensions(dimensions) ? dimensions : null,
        ...(item ? { ...currentEditMeta(), quantity: 1 } : { ...currentCreateMeta(), volume: snapshot.volume, nestable: snapshot.nestable }) },
      localFormContext: { version: 1, ...(item ? { availabilityStatus: snapshot.availabilityStatus } : {}), placement },
      ...
      unsupportedPlacement: Boolean(pendingCopyTargetContainerSetup || placement && placement.layoutId !== layoutId
        || !item && (runtime.rootContainerDialogPendingParentId != null || runtime.rootContainerDialogPendingParentIndex != null
          || runtime.rootContainerDialogPendingRootIds != null)) };
  },
  submit: submitAdminTemplatePhotoCreateForm,
  ...adminTemplatePhotoFormCallbacks
});
...
const adminTemplatePhotoForms = createAdminTemplatePhotoFormController({
  ...
  readForm(type) {
    ...
    return { created: !entityId,
      fields: { name: snapshot.name, weight: snapshot.weight, color: snapshot.color, location: snapshot.location,
        category: categories[0] || "", categories, note: snapshot.note, noteHtml: snapshot.noteHtml || "",
  ...
  submitEdit: submitAdminTemplatePhotoEditForm,
  ...adminTemplatePhotoFormCallbacks
});

async function prepareOpenedFormPhotos(type, files) {
  const created = await adminTemplatePhotoCreateForms.preparePhotos(type, files);
...
function adminTemplateAnyPhotoFormEnabled() {
  return adminTemplatePhotoFormEnabled() || adminTemplatePhotoEditFormEnabled() || adminTemplatePhotoCreateFormEnabled();
}

function openedPhotoFormInputGuard(type) {
  return adminTemplatePhotoCreateForms.inputGuard(type) || adminTemplatePhotoForms.inputGuard(type) || personalPhotoForms.inputGuard(type);
}
...
const personalPhotoForms = createPersonalPhotoFormController({
  ...
  getView(type) {
    const item = type === "item";
    return { token: item ? runtime.itemDialogInitialSnapshot : runtime.rootContainerDialogInitialSnapshot,
      dialog: item ? refs.dialog : refs.rootContainerDialog,
      saveButton: item ? refs.saveItemBtn : refs.saveRootContainerBtn,
      signature: JSON.stringify(item ? getItemDialogSnapshot() : getRootContainerDialogSnapshot()),
      draft: item ? runtime.itemDialogPhotoDraft : runtime.rootContainerDialogPhotoDraft,
  ...
  readForm(type, { pendingUpdate = false, pendingFiles = false, pendingCreate = false } = {}) {
    ...
    const categories = item ? getDialogSelectedCategories() : getRootContainerDialogSelectedCategories();
    const dimensions = item ? readItemDialogDimensions() : readRootContainerDialogDimensions();
    const fields = { name: snapshot.name, weight: snapshot.weight, color: snapshot.color,
      location: snapshot.location, category: categories[0] || "", categories, note: snapshot.note, noteHtml: snapshot.noteHtml || "",
      dimensions: hasContainerDimensions(dimensions) ? dimensions : null,
      ...(created ? currentCreateMeta() : currentEditMeta()),
      ...(item ? { quantity: 1, stockQuantity: snapshot.stockQuantity, stockLocations: snapshot.stockLocations } : { volume: snapshot.volume, nestable: snapshot.nestable }) };
  ...
  onQueued: () => scheduleRemoteSave(),
  onError: reportPersonalPhotoFormError,
  onBusy(type, active) {
    const button = type === "item" ? refs.saveItemBtn : refs.saveRootContainerBtn;
    if (active && button) { button.disabled = true; button.textContent = localText("Saving…", "Сохраняю…"); }
    else if (type === "item") updateItemDialogSaveState(); else updateRootContainerDialogSaveState();
...
async function handleItemPhotoInputChange(event) {
  ...
  try {
    ...
    if (error.isStalePhotoForm) return;
...
async function removeItemDialogPhoto() {
  ...
  const isCurrent = (adminTemplatePhotoCreateForms.mutationGuard("item") || adminTemplatePhotoForms.mutationGuard("item")), selectedIndex = runtime.itemDialogPhotoActiveIndex;
  const confirmed = await confirmDialogPhotoDelete("item");
  if (!confirmed || isCurrent && (!isCurrent() || runtime.itemDialogPhotoActiveIndex !== selectedIndex)) return;
  const result = removePhotoFromDraft(draft, selectedIndex, source);
  runtime.itemDialogPhotoDraft = result.draft;
  runtime.itemDialogPhotoActiveIndex = result.nextIndex;
  if (result.discardedPhoto) {
    deleteCachedDraftPhotos(result.discardedPhoto);
    if (!adminTemplateAnyPhotoFormEnabled() && !adminTemplatePhotoForms.owns("item") && (result.discardedPhoto.url || result.discardedPhoto.thumbUrl)) {
...
function openPhotoOrderDialog(kind = "item") {
  ...
  photoOrderContext = { kind, photos: [...draft.photos], activeId: photoOrderIdentity(draft.photos[activeIndex]), previewSources,
    isCurrent: adminTemplatePhotoCreateForms.mutationGuard(kind) || adminTemplatePhotoForms.mutationGuard(kind) };
...
async function requestClosePhotoOrderDialog() {
  const requestedContext = photoOrderContext;
  if (!hasSavablePhotoOrderChanges()) {
    refs.photoOrderDialog.close("cancel");
    return;
  }
  const action = await askUnsavedChangesDialog();
  if (photoOrderContext !== requestedContext || requestedContext?.isCurrent && !requestedContext.isCurrent()) return;
  if (action === "save") {
...
function applyPhotoOrderDialog(event) {
  event?.preventDefault();
  if (!photoOrderContext) return;
  if (photoOrderContext.isCurrent && !photoOrderContext.isCurrent()) {
    refs.photoOrderDialog.close("cancel");
    return;
  }
...
function cleanupUnsavedItemDialogPhotoDraft() {
  ...
  draftPhotosToCleanup(runtime.itemDialogPhotoDraft, source).forEach((photo) => {
    if (!adminTemplateAnyPhotoFormEnabled() && !adminTemplatePhotoForms.owns("item") && (photo?.url || photo?.thumbUrl)) deleteRemotePhotoIfPossible(entityId, photo);
...
async function updateItemDialogPhotoPreview(photos) {
  ...
  setItemDialogPhotoStatus(openedFormPhotoStatus(list));
}
...
async function removeRootContainerDialogPhoto() {
  ...
  const draft = runtime.rootContainerDialogPhotoDraft || createPhotoDraftFromRecord(source);
  if (!draft.photos.length) return;
  const isCurrent = (adminTemplatePhotoCreateForms.mutationGuard("container") || adminTemplatePhotoForms.mutationGuard("container")), selectedIndex = runtime.rootContainerDialogPhotoActiveIndex;
  const confirmed = await confirmDialogPhotoDelete("container");
  if (!confirmed || isCurrent && (!isCurrent() || runtime.rootContainerDialogPhotoActiveIndex !== selectedIndex)) return;
  const result = removePhotoFromDraft(draft, selectedIndex, source);
  runtime.rootContainerDialogPhotoDraft = result.draft;
  runtime.rootContainerDialogPhotoActiveIndex = result.nextIndex;
  if (result.discardedPhoto) {
    deleteCachedDraftPhotos(result.discardedPhoto);
    if (!adminTemplateAnyPhotoFormEnabled() && !adminTemplatePhotoForms.owns("container") && (result.discardedPhoto.url || result.discardedPhoto.thumbUrl)) {
      deleteRemotePhotoIfPossible(
...
async function uploadItemDialogDraftPhotos(photos = []) {
  if (adminTemplateAnyPhotoFormEnabled() || adminTemplatePhotoForms.owns("item") || adminTemplatePhotoCreateForms.owns("item")) return false;
...
async function uploadRootContainerDialogDraftPhotos(photos = []) {
  if (adminTemplateAnyPhotoFormEnabled() || adminTemplatePhotoForms.owns("container") || adminTemplatePhotoCreateForms.owns("container")) return false;
  ...
  await uploadDialogDraftPhotos({
    ...
    onPhotoProgress: (photo) => {
      ...
      setRootContainerDialogPhotoStatus(openedFormPhotoStatus(list));
    },
    onAfterUpload: () => {
      const list = runtime.rootContainerDialogPhotoDraft?.photos || normalizeItemPhotos(container);
      updatePhotoGalleryUploadProgress(refs.rootContainerPhotoPreview, list);
...
function cleanupUnsavedRootContainerDialogPhotoDraft() {
  ...
  draftPhotosToCleanup(runtime.rootContainerDialogPhotoDraft, source).forEach((photo) => {
    if (!adminTemplateAnyPhotoFormEnabled() && !adminTemplatePhotoForms.owns("container") && (photo?.url || photo?.thumbUrl)) deleteRemotePhotoIfPossible(entityId, photo, "container");
    deleteCachedDraftPhotos(photo);
  });
}
...
function updateItemDialogSaveState() {
  ...
  if (itemFormDraftSaving || adminTemplatePhotoCreateForms.busy("item") || adminTemplatePhotoForms.busy("item") || personalPhotoForms.busy("item")) refs.saveItemBtn.disabled = true;
...
function updateRootContainerDialogSaveState() {
  ...
  if (rootContainerFormDraftSaving || adminTemplatePhotoCreateForms.busy("container") || adminTemplatePhotoForms.busy("container") || personalPhotoForms.busy("container")) refs.saveRootContainerBtn.disabled = true;
...
async function saveRootContainerDialog(event) {
  ...
  if (adminTemplatePhotoCreateForms.save("container")) return;
...
async function saveDialogItem(event) {
  ...
  if (adminTemplatePhotoCreateForms.save("item")) return;
...
function applyItemDialogPhotoDraft(item, changedAt = nowIso()) {
  ...
  if (adminTemplateAnyPhotoFormEnabled() || adminTemplatePhotoForms.owns("item") || adminTemplatePhotoCreateForms.owns("item")) throw Error("Сохраните фотографии через исходную административную форму.");
...
function applyRootContainerDialogPhotoDraft(container, changedAt = nowIso()) {
  ...
  if (adminTemplateAnyPhotoFormEnabled() || adminTemplatePhotoForms.owns("container") || adminTemplatePhotoCreateForms.owns("container")) throw Error("Сохраните фотографии через исходную административную форму.");
