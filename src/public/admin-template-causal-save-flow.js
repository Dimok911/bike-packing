...
const planSnapshot = plan => [2, 3, 5, 6, 7].includes(plan.version) ? plan.editorSnapshot
...
function savedSuccessor(observed, saved) {
  ...
  for (;;) {
    ...
    if ([6, 7].includes(latest?.plan.version)) throw recoveryRequired();
...
export function createAdminTemplateSaveFlow({ getLayout, getContext, snapshot, plansFor, recoveryFor = null, resolutionFor = null,
  applyServerVariant = null, applyPhotoResult = null, applyPhotoEditResult = null, applyPhotoCreateResult = null, persist, notify = () => {},
  ...
  const capture = async (layoutId, { published = null } = {}) => {
    ...
    if (observed.photoAppendPending || observed.photoEditPending || observed.photoCreatePending) throw blocked();
  ...
  const flush = async layoutId => {
    ...
    if (ordered.slice(0, -1).some(row => [6, 7].includes(row.plan.version))) throw recoveryRequired();
    let result;
    ...
    if ([5, 6, 7].includes(plan.version)) {
      const apply = plan.version === 7 ? applyPhotoCreateResult : plan.version === 6 ? applyPhotoEditResult : applyPhotoResult;
      delete nextSource[plan.version === 7 ? "photoCreatePending" : plan.version === 6 ? "photoEditPending" : "photoAppendPending"];
