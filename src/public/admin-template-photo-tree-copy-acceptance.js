...
import { validateAdminTemplatePhotoTreeCopyReceipt } from "../sync/admin-template-photo-tree-copy-receipt.js";
import { prepareAdminTemplatePhotoTreeCopyProjection } from "./admin-template-photo-tree-copy-projection.js";

export const ADMIN_TEMPLATE_PHOTO_TREE_COPY_ACCEPTANCE_PREFIX = "bike-packing-admin-photo-tree-copy-accepted-v1:";
const kind = "admin-template-photo-tree-copy-accepted", collections = ["layouts", "items", "containers"];
...
const pause = code => { throw Object.assign(Error("Подтверждённая копия дерева ещё требует проверки принятия."),
  { code: `admin-template-photo-tree-copy-acceptance-${code}`, isAdminTemplateBlocked: true }); };
const sync = fn => {
  const result = fn();
...
export function adminTemplatePhotoTreeCopyAcceptanceKey(binding, operationId) {
  if (!validTemplateOperationId(operationId)) pause("operation");
  return ADMIN_TEMPLATE_PHOTO_TREE_COPY_ACCEPTANCE_PREFIX + encodeURIComponent(canonical(adminTemplatePhotoActionBinding(binding))) + ":" + operationId;
}
...
function scope(input, externalGuard) {
  ...
  const operationId = input.operationId, key = adminTemplatePhotoTreeCopyAcceptanceKey(binding, operationId);
...
async function fullProof(s, extraGuard = () => {}) {
  ...
  if (!exact(saved, ["version", "plan", "digest", "cancelRequested"]) || saved.version !== 1 || saved.cancelRequested !== false
    || saved.plan?.version !== 9 || saved.plan.id !== s.operationId || !same(saved.plan.binding, s.binding)) pause("plan");
  const planDigest = await digest(saved.plan); guard(); if (saved.digest !== planDigest) pause("plan");
  const record = await assertAdminTemplatePhotoTreeCopyPlanRecord(saved.plan, s.store, guard); guard();
  ...
  if (!(exact(journal, keys) || exact(journal, [...keys, "cancelRequested"])) || journal.version !== 1
    || journal.kind !== "admin-template-photo-tree-copy" || typeof journal.dispatched !== "boolean"
  ...
  const valid = await validateAdminTemplatePhotoTreeCopyReceipt(journal.receipt,
  ...
  // Reconstruct only historical before namespaces; an ordinary save may have
  // legitimately changed the current editor since acceptance. No current field
  // is normalized or substituted for the immutable source/target record.
  const { source, target } = record.snapshot, state = copy(target.beforeState);
  for (const type of collections) state[type] = { ...copy(source.beforeState[type]), ...copy(target.beforeState[type]) };
  const projection = await prepareAdminTemplatePhotoTreeCopyProjection({ state, plan: saved.plan, store: s.store,
...
// exact V9 plan, full command journal, all stages/paths and committed projection.
...
export async function readAdminTemplatePhotoTreeCopyAcceptance(input, externalGuard) {
...
export async function inspectAdminTemplatePhotoTreeCopyAcceptanceCandidate(input, externalGuard) {
  if (!exact(input, readKeys)) pause("dependencies");
...
export async function prepareAdminTemplatePhotoTreeCopyAcceptance(input, externalGuard) {
