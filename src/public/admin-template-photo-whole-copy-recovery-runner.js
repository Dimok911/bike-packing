...
import { validateAdminTemplatePhotoWholeCopyStageReceipt, validateAdminTemplatePhotoWholeCopyStages,
  validateAdminTemplatePhotoWholeCopyReceipt } from "../sync/admin-template-photo-whole-copy-receipt.js";
...
const fail = code => { throw Object.assign(Error("Сохранённая копия укладки требует повторной сверки."),
  { code: `admin-template-photo-whole-copy-recovery-${code}`, isAdminTemplateBlocked: true }); };
const sync = value => {
...
// BikePacking recovery of an EXISTING V10 only. The app fixes transport and flags
...
export function createAdminTemplatePhotoWholeCopyRecoveryRunner({ binding, layoutId, getContext, store, plans, createClient,
  ...
  const journalPrefix = "bike-packing-admin-photo-whole-copy-commands-v1:" + suffix;
  const context = () => {
  ...
  const execute = async (operationId, mode) => {
    ...
    if (!exact(saved, ["version", "plan", "digest", "cancelRequested"]) || saved.version !== 1
      || saved.cancelRequested !== false || saved.plan?.version !== 10 || saved.plan.id !== operationId
    ...
    const prove = async guard => {
      ...
      guard(); const record = await assertAdminTemplatePhotoWholeCopyPlanRecord(saved.plan, store, guard); guard();
      if (record.snapshot.source.layoutId !== layoutId || !same(record.binding, binding)) fail("target");
      return record;
    };
    try {
      baseGuard(); const first = await prove(baseGuard); baseGuard();
      const source = first.action.body.source;
      ...
      return await withAdminTemplateCapture({ bindings, locks }, async captureLease => {
        ...
        const readJournal = async () => {
          ...
          if (!(exact(journal, keys) || exact(journal, [...keys, "cancelRequested"]))
            ...
            || journal.version !== 1 || journal.kind !== "admin-template-photo-whole-copy" || !same(journal.intent, intent)
          ...
          for (const [index, receipt] of journal.stageReceipts.entries()) if (receipt !== null) {
            const valid = await validateAdminTemplatePhotoWholeCopyStageReceipt(receipt, { manifest: record.stages[index], assetDigest: assets[index].assetDigest }); check();
          ...
          if (journal.stageReceipts.every(stage => stage !== null)) {
            const valid = await validateAdminTemplatePhotoWholeCopyStages(intent, journal.stageReceipts); check(); if (!valid) fail("stages");
          }
          if (journal.receipt !== null) {
            const valid = await validateAdminTemplatePhotoWholeCopyReceipt(journal.receipt, { intent, payloadDigest, stageReceipts: journal.stageReceipts }); check();
