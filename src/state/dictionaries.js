...
function migratePrivateCustomDictionary(owner, type, defaults = {}, helpers = {}) {
  ...
  const legacyValues = normalizeDictionaryValues(owner[type === "location" ? "locations" : "categories"]);
...
function migrateLayoutCustomDictionary(layout, type, sourceState, defaults = {}, helpers = {}) {
  ...
  layout[type === "location" ? "customLocations" : "customCategories"] = [];
}
...
export function ensureLayoutDictionaries(layout, {
  ...
  layout.locations = normalizeDictionaryValues([
    ...customDictionaryValues(layout, "location"),
    ...usedLocations
  ], defaults.locations || []);
  layout.categories = normalizeDictionaryValues([
    ...customDictionaryValues(layout, "category"),
    ...usedCategories
  ], defaults.categories || []);
  return layout;
}
...
export function readOnlyLayoutDictionaries(layout, {
  ...
  const helpers = { getLayoutContainerIdSet, getLayoutItemIdSet };
  return {
    locations: normalizeDictionaryValues(layoutDictionaryValues(layout, "location", source, helpers), defaults.locations || []),
    categories: normalizeDictionaryValues(layoutDictionaryValues(layout, "category", source, helpers), defaults.categories || [])
  };
}
