...
export function createManufacturerBagCatalogDialogController({
  ...
  function renderProductList(entries) {
    ...
    return `
      <div class="manufacturer-catalog-products">
        ${entries.map(renderProductCard).join("")}
      </div>
    `;
  }
  ...
  function renderProductCard(entry) {
    ...
    return `
      <article class="manufacturer-catalog-product">
        <a class="manufacturer-catalog-product-image" href="${escapeHtml(entry.sourceUrl)}" target="_blank" rel="noopener noreferrer">
          <img src="${escapeHtml(entry.imageUrl)}" alt="${escapeHtml(`${entry.brand} ${entry.name}`)}" loading="lazy" />
        </a>
        <div class="manufacturer-catalog-product-body">
          ...
          <div class="manufacturer-catalog-product-actions">
            <a class="ghost manufacturer-catalog-source-link" href="${escapeHtml(entry.sourceUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(t("bagCatalog.source"))}</a>
            ${canEdit() ? `<button class="ghost" type="button" data-bag-catalog-edit="${escapeHtml(entry.id)}">${escapeHtml(t("bagCatalog.edit"))}</button>` : ""}
            <button type="button" data-bag-catalog-select="${escapeHtml(entry.id)}">${escapeHtml(t("bagCatalog.use"))}</button>
          </div>
        </div>
      </article>
    `;
  ...
  function applyEditorLabels() {
    ...
    labels.forEach(([input, key]) => {
      ...
      const text = label?.querySelector?.(".bag-catalog-edit-label");
