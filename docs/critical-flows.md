...
- На iOS Safari касание, останавливающее нативную инерционную прокрутку, может
  ...
  контейнера.
- На iOS/iPadOS viewport изолируется в стабильной
  `.app[data-viewport-scroll-host]`, а единственная интерактивная кнопка
  добавляется непосредственно в `body` как сосед `.app`. В повторно
  отрисовываемых панелях «Вещи» и «Сумки» остаются только позиционные якоря.
- Scroll-host включается до первой отрисовки и привязки touch-обработчиков.
  Нельзя поздно переносить владение прокруткой между document и `.app`: Safari
  может сохранить focus/sticky-геометрию от одного host и начать momentum на
  другом. Позиции отдельных вкладок программно не запоминаются и не
  восстанавливаются.
...
- Контракт защищён `tests/critical/catalog-back-to-top.test.js`,
  ...
  `tests/critical/sticky-scroll.test.js`.
