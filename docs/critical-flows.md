...
- Общее локализованное имя автоматической гостевой копии (`Demo layout` /
  `Демо-укладка`) перед импортом заменяется известным именем исходного шаблона
  из handoff (`fallbackName`), и уже к нему применяется обычная порядковая
  уникализация имени.
