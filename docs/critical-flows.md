...
- Короткое первое касание выполняет ровно один мгновенный переход наверх без
  предварительного события `scroll`; последующий синтетический `click`
  подавляется. Перемещение больше порога отменяет действие.
...
- Контракт защищён `tests/critical/catalog-back-to-top.test.js`,
  ...
  первого browser-routed scroll в каждой обычной вкладке.
