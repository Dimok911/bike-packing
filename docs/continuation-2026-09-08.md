...
Текущий ПРИНЯТЫЙ локально срез: новые файлы в следующей ожидающей фотоформе той же
вещи/сумки. API 2d36554; frontend — коммит с этой записью. Предыдущая FE основа a21d4d4.
В рабочем дереве подключены compiler, полная цепочка форм, собственный выключенный
флаг ownerResult, native storage/session/controller, outbox/queue/drain, серверное
разрешение реальных ревизий под блокировкой и DB-потомки photoResults.version=6.
Пройдены 632 transport (pending-files-transport-5.log), 896 critical
(pending-files-critical-1.log), FE check-4, API 92 operations и check-2/74 source.
UI-6 отмена 4/4 Chromium; UI-7 расширенные 28/28, 8,4 минуты, завершены.
Регрессия-1 18/18 прошла. Gallery UI-1 остановился на ожидании движения за один
rAF: тест теперь ждёт фактически отрисованного движения не более четырёх кадров,
сохраняя строгие проверки движения до конца анимации и немедленного pinch.
Повтор gallery UI-2 запущен. API/MySQL-4 завершён: 242/242,
351,10 с, exit0; MySQL остановлен. API сохранён коммитом 2d36554.
Следующее: закончить gallery UI, сохранить проверенные коммиты, затем подключить
fileless изменение самих pending-фотографий и продолжать ВЕСЬ checklist. Никакой публикации.
Галерея 2.4.1 уже перенесена локально, runtime SHA256 сверён; прежний hold снят
отдельным согласованием принятой v1603. Публикацию это здесь не разрешает.
...
Этап производителя подключён и проверен в UI/outbox/queue/API,
включая удаление всех выбранных фотографий перед SAVE, размещение, последующие
DB-поля и отмену fileless формы. Полный источник, выбранные URL и файлы сохраняются
неизменно; источник проходит JSON-проверку до clone/fetch. FE/BE чистый source
parser имеет одинаковый SHA256 41E061632DC6FCCF1F16087901E00A0462268A1E239339EA9307B0F98FC575B4.
Оба собственных флага false. Документы: personal-manufacturer-photo-forms.md,
API causal-manufacturer-photo-forms.md. Исходные данные каталога не менялись.

Проверены 614 transport (manufacturer-transport-4.log), 896 critical
(manufacturer-critical-4.log), FE check-5, API 83 operations и 74 source/service.
Настоящий API/MySQL 235/235, 345,80 с (manufacturer-mysql-2.log), exit0,
MySQL остановлен. Итоговый UI-4 20/20, 3,4 мин; регрессия-4 18/18, 4,1 мин.
Процессы exit0, завершены. API-коммит d9a48e0; frontend — коммит с этой записью.
Регрессия-2/debug воспроизвела ошибку прежнего pending-form среза в photo-form
режиме: personalPendingImportSource возвращал false вместо null, что ломало
чтение action.kind до отправки. Исправлен контракт helper в app.js. Регрессия-3
прошла 9 Chromium проверок и остановилась в WebKit на старом каталожном пути:
теперь отдельный выключенный source gate останавливает выбор ДО загрузки фото,
не входит в legacy Blob cache, сохраняет форму и показывает предупреждение.
Первый тест регрессии также уточнён: до регистрации действия ожидается
предупреждение, а не recovery-окно. Проверки безопасности не ослаблены.

Следующий открытый путь — добавление новых файлов в следующей ещё ожидающей
форме того же владельца. Нужны собственная точная ссылка на результат владельца,
сохранённые новые bytes/UUID и числовые версии из подтверждённой транзакции;
нельзя подставлять фиктивную revision 1 в отправляемый запрос или повторно
читать более новый источник. Дальше публичные копии, sharing, admin и общий аудит.
Текущий код подключён на FE/API, включая отмену всех форм в обратном порядке,
а затем DB-потомков, если хотя бы одно фотодействие отклонено. Все исходные
файлы сохраняются. 629 transport и FE check прошли до последней правки compiler;
92 API operations и API check прошли. UI-1 нашёл ошибочное runtime.editingItemId
в app.js; UI-2 обнаружил конфликт временного synced статуса с реальным serializer.
Обе причины исправлены; добавлен тест настоящего project вместо identity callback.
UI-3 и API/MySQL-1 запущены, окончательная приёмка нового среза ещё не выполнена.
Новое согласование галереи: source task передала принятую v1603 / gallery 2.4.1;
локальный перенос запланирован после активных тестов. См. experiment-gallery-2.4.0-local.md.
