...
- Следующий участок — локальное удаление источника или самой копии, пока её
  точное подтверждение ещё не пришло. Затем массовые/деревья/укладки/архивы/
  обмен/admin и прочие открытые пункты. Не завершать работу после одиночной копии.
  Важная граница: `pending` ссылки копии нельзя записывать в SQL как готовые,
  подбирать URL или заново читать изменившийся источник. Обсуждённый вариант
  следующего адаптера (ещё НЕ реализован): immutable `photoResults` в дочернем
  list.update связывает только точный результат копии; сервер после проверки
  declared dependency и CAS подставляет ссылки из прежней committed-квитанции
  в отдельный effective payload. Исходный body/UUID/hash остаются неизменны.
  Нужны отдельный gate, строгий chain validator, поддержка drain/recovery/cancel,
  reverse delivery/lost ACK/отмена зависимого удаления и проверки подделок.
