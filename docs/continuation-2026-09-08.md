...
- Следующее расширение: поля + удаления + перестановка оставшихся фото в одной
  ...
  74 source/service. Frontend ещё не закоммичен: 418 transport, 892 critical
...
- Trello: пользователь прямо подтвердил в этой беседе «Да, передавать
  ...
  сохранила и перечитала карточку 06 с результатами `701a8f2`/`bc579c8` и 28/28.
  Новое смешанное расширение пока не передано. Карточка открыта, 0/4.
...
- Дополнительное поручение пользователя передано из задачи Safari: выровнять
  ...
  совпала с live по styles/sw/contract/manifest, но app/index ещё требуют сверки.
...
- Не передавать повторно, не перемещать и не удалять размещённые фото/их каталоги.
  ...
  Production и API не публиковать. Перенос исправлений ещё не сделан.
