...
Начата отдельная подготовка API: новые НЕПОДКЛЮЧЁННЫЕ
src/lib/bike-packing-photo-form-owner-result.js и одноимённый test, 3/3.
Они не входят в d9a48e0. Чистый resolver проверяет точного непосредственного
предшественника и полный owner со смешанными synced/pending photos, возвращает
только отдельное доказательство для серверного контекста. UUID/body/revision
не переписывает, числовую версию владельца ещё должен проверить handler под SQL
lock. Нет UI, формата локальной цепочки с несколькими файлами, HTTP подключения,
stage/drain/cancel, потомков версии 6 и paired API для этого нового этапа.
