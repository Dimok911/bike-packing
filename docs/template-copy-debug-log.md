...
## v653: удаление считаем разобранным

- Причина оставшегося "не удаляется" была во фронтовом критерии: после refresh опубликованная копия открывалась как обычный `adminSharedSourceId` из runtime/index, уже без флага `adminTemplateCopy`. Старый код удалял с сервера только `adminTemplateCopy`, поэтому переоткрытый published shared-шаблон удалялся только локально.
