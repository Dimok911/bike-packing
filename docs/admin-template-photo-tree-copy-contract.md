# Копирование дерева с фотографиями: контракт и проверка

Актуально на 12.09.2026: функциональная разработка карточки [3.2](https://trello.com/c/dXKFGpqp) завершена локально в указанной ниже границе. Совместная форма с настоящими Auth/API/MySQL и файлами прошла все 12 сценариев Chromium/mobile WebKit; общий набор обмена — 2168/2168, базовые проверки — 918/918. Флаги исходников остаются OFF, в Experiment v1610 этот результат не входит. Скорость и отзывчивость настоящего iPhone/Safari не приняты и остаются этапом 9. Целые шаблоны и остальные направления копирования — отдельные карточки 3.3 и 3.4.

Ниже сохранена история последовательных срезов. Формулировки «ещё требуется» внутри прежних срезов относятся к их тогдашнему состоянию; окончательное evidence находится в последнем разделе.

## История развития контракта
...
## Совместная браузерная проверка с настоящими службами

Добавлен opt-in набор `tests/integration/admin-template-photo-tree-copy-browser.mjs`, вызываемый парным серверным `admin-template-photo-tree-copy-browser-scenarios.js`. Он открывает настоящие формы в Chromium и mobile WebKit, а разрешённые canonical API-запросы направляет в отдельные локальные Auth, Tags, Bike API и MySQL. Ответы бизнес-команд и фотографии поступают от этих служб. Две внешние Shared UI зависимости намеренно недоступны; проверяется использование настоящих встроенных версий. Локальные blob-адреса изображений исполняет сам браузер. Ни рабочая сессия пользователя, ни опубликованные службы в наборе не используются.

Шесть сценариев в каждом движке: полное копирование с декодированием превью и оригинала, обычной правкой, перезагрузкой и второй копией; потерянный ответ commit; отказ native IDB и квота mirror; прерывание записи acceptance после подтверждённого mirror; явная отмена незавершённых стадий; поздний commit после намерения отменить. Холодное восстановление проверяется с выключенными tree gates клиента и настоящего API. Проверяются полные raw IDB records, исходные claims, планы, журналы, все receipts, accepted projections, SQL, история и отдельные физические файлы. Независимый личный список должен действительно загрузиться и остаться неизменным. Повторные бизнес- и stage POST запрещены там, где исходная операция уже выполнена.

Совместная проверка выявила и позволила исправить рабочие ошибки: серверные количества ошибочно обрабатывались как старый локальный формат; несериализуемое перечислением свойство выбора укладки терялось в защитном снимке; X формы блокировался старым дробным весом; административные шаблоны попадали в выбор личного списка; внешний `listId` ответа `/state` терялся при извлечении вложенного record. Исправления сохраняют прежнюю обработку настоящих локальных данных, приоритет вложенного ID и проверку формы при сохранении.

Отдельно исправлено обнаружение mirror-only принятия: после записи подтверждённого mirror, но до acceptance, `planId` и pending-поля уже отсутствуют. Открытие существующего шаблона теперь вызывает отдельный V9 resume и без указателя, а явный Sync ищет полную сохранённую запись перед выбором окна восстановления. После асинхронного поиска Sync повторно проверяет аккаунт, поколение, выбранный шаблон и identity редактора. Обычный путь без V9 и уже принятые операции не создают новые capture/flush. Новые регрессии вызывают настоящие open/Sync, а не напрямую обходящий обнаружение resume.

Диагностические матрицы `tree-browser-matrix-3` и `tree-browser-matrix-4` завершили по 10 из 12 сценариев. После исправления вызова обнаружения два quota-отказа сохранились: холодное применение размещения меняло ровно один корневой `parentId` с `""` на `null`. Проверка точной принятой проекции правильно отказывалась считать такое зеркало исходным результатом. В matrix-4 все 6238 FE и 340 BE файлов и оба HEAD были неизменны, disposable MySQL остановлена с подтверждением закрытия порта 64355. Это диагностическое evidence, не окончательная приёмка.

Читающий адаптер размещения теперь сохраняет существующее представление отсутствующего родителя при неизменном размещении; настоящая смена родителя по-прежнему применяется. Сохранение размещения, словарей и полного каталога подтверждённого редактора больше не зависит от флага создания новых фото-команд. Независимое воспроизведение с настоящими функциями применения размещения и восстановления и V9-проекцией подтверждает неизменность source/target также при всех выключенных photo writers. OFF браузерная сборка проверяет именно этот режим. Общие guards, правила приёма и разрешения новых команд не ослаблены. Окончательные проверки исправления ещё выполняются; карточка 3.2 остаётся «В работе».

Производительность не принята: в mobile WebKit на Windows вторая копия превысила 120 секунд, продолжая проходить последовательные стадии. Трасса показала около 43 секунд локальной подготовки и около 21 секунды между стадиями при ответах сервера 46–57 мс. Только ожидание второй копии этого теста увеличено до 240 секунд, остальные проверки сохранены. Это не ускоряет приложение. До приёмки на настоящем iPhone требуется измерить время и отзывчивость для разных размеров дерева и накопленной истории; при подтверждении задержки оптимизировать повторные проверки без ослабления доказательств. Требование записано в карточку этапа 9.

Адресный прогон `tree-browser-acceptance-quota-5` после этого исправления прошёл в Chromium, но WebKit выявил второй путь изменения: при всех выключенных photo writers фоновое открытие другого, ещё не отображённого административного шаблона использовало legacy-обработчик и меняло соседние подтверждённые каталоги. Прогон был неизменным (6238 FE / 340 BE), MySQL остановлена, порт 49721 закрыт; это диагностический результат 1 из 2 браузерных сценариев.

Теперь синхронный legacy-обработчик временно получает коллекции без неактивных подтверждённых каталогов с photoOwnerMap. Сам объект состояния сохраняется, поэтому прежняя отметка миграции количеств и packedItems работают на исходном состоянии; исходные ссылки трёх коллекций возвращаются в finally. Ссылки на защищённые записи в чужих размещениях отклоняются до изменения состояния. Такие записи также не попадают в автоматический поиск корневых сумок. Точное воспроизведение последовательности WebKit с настоящими функциями активации подтверждает сохранение source/target, исходных объектов и runtime-геттера выбора укладки. Браузерная приёмка этого исправления ещё предстоит.

## Итог совместной функциональной проверки — 12.09.2026

Финальный `tree-browser-matrix-5` прошёл **12/12 браузерных сценариев** (6 Chromium и 6 mobile WebKit), без повторов, ошибок, отмен или пропусков. Node итог — 13/13 с родительским API-тестом, 1161,388 секунды. Проверены две копии с обычной правкой и холодной загрузкой, открытие настоящего фото, потеря успешного ответа, ошибки IDB/mirror/acceptance, явная отмена и поздний commit. При холодном восстановлении отключены все четыре клиентских photo writers и снята server TREE capability; чтение и принятие исходной операции сохраняются без новых UUID и повторных POST.

Обе рабочие версии поверх FE `84f789ca` и BE `ff0680e8` были неизменны: 6238 FE и 340 BE файлов, проверка 17:17:29.409 UTC. Одноразовая MySQL остановлена, порт 64422 закрыт. Логи, снимки исходников и verified JSON находятся в BE `node_modules/.cache/tree-browser-matrix-5*`; браузерные JSON и traces — FE `test-results/tree-browser-matrix-5`. ON/OFF build11 завершены до прогона; дополнительное сравнение всех 4865 файлов каждой сборки с 17:00:50 UTC до завершения также не выявило изменений. Этот снимок сборок снят во время матрицы, а не до её начала.

На том же runtime прошли **2168/2168** проверок обмена за 538,322 секунды (`tree-browser-transport-2026-09-12T16-48-17-324Z-*`, 6236 файлов и HEAD неизменны; два browser-only файла проверены отдельной матрицей) и **918/918** базовых проверок за 2,045 секунды (`tree-browser-critical-2026-09-12T17-18-06-923Z-*`, все 6238 файлов неизменны). Source check, обе сборки и diff check пройдены. Focused и прежние диагностические прогоны не прибавляются к этим числам. После проверок менялась только документация результатов.

Контрольные SHA-256: `app.js` — `e780b0ef14402302858926f6e89d5e17fda3033c01e9c9e6105ce20b3c9f9bfb`; FE browser module — `88211041ff4e345f60c68b38fa70fa5e2356ba1a14e782f7f87a919825b526d5`; BE scenarios — `1b1ab294f8055301a59a0dec49f0435353ac4c312fa33347c84e4af7a348bd1a`.

Это завершает функциональную локальную границу карточки 3.2, но не публикацию, весь этап 3 или приёмку скорости. В финальном WebKit полный positive-сценарий с двумя копиями занял 307,821 секунды. Четыре запроса стадий второй копии занимали 43–62 мс каждый, интервалы между ними — около 21–22 секунд. Нужны отдельное измерение повторных локальных проверок и приёмка на настоящем iPhone в этапе 9. GitHub Actions не запускался; исходные флаги OFF, опубликованный Experiment v1610 не менялся.