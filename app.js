...
async function init() {
  ...
  window.addEventListener("offline", () => {
    ...
    if (isExplicitlySignedOut() || !rememberedUser) {
      enterSignedOutPublicMode("Offline · personal lists are hidden, local demo copy is open").catch(() => {
        setActiveReadOnlyScope(DEMO_SHARED_LAYOUT_ID);
        render();
        updateSyncUi("Offline · personal lists are hidden, demo/public template is open");
      });
      return;
    }
    ...
    updateSyncUi("Offline · local layout is available");
  });
  ...
  if (sharedListId) {
    ...
    if (signedOut) enterSignedOutPublicMode("Signed out · personal lists are hidden, local demo copy is open");
    else unlockOfflineState("Forced offline · local layout is available");
  } else if (offlineNow) {
    publicIndexRefresh.catch(() => null);
    if (!activateOfflineRememberedSession("Offline · local copy of personal layouts is open")) {
      enterSignedOutPublicMode("Offline · sign-in is not confirmed, local demo copy is open");
    }
  } else {
    publicIndexRefresh.catch(() => null);
    checkAuthAndLoad();
...
function setLayoutLoadProgress({ loaded = 0, total = null, prefix = "Loading personal layouts" } = {}) {
...
function activateOfflineRememberedSession(message = "Offline · local copy of personal layouts is open") {
  ...
  setActivePrivateScope();
  setLayoutLoadStatus("warning", "Offline: showing the local copy of personal layouts");
...
async function toggleForcedOfflineMode() {
  ...
  if (next) {
    ...
    currentUser = null;
    appUnlocked = true;
    activateOfflineRememberedSession("Forced offline · local layout is available");
    updateSyncUi("Forced offline · local layout is available");
    showToast("Offline mode is on. The API will not be used.", "success");
    return;
  }
  clearOfflineRememberedSession();
  updateSyncUi("Offline mode is off · checking sign-in...");
  showToast("Offline mode is off. Sync is available.", "success");
...
function openAuthDialog() {
  ...
  refs.authSubmitBtn.textContent = "Send link";
...
async function submitAuthDialog(event) {
  ...
  if (isForcedOffline()) {
    ...
    refs.authDialogStatus.textContent = "Offline mode is on. Turn it off in the menu to sign in.";
    updateSyncUi("Forced offline · sign-in is disabled");
    return;
  }
  const email = refs.authEmail.value.trim();
  if (!email) {
    refs.authDialogStatus.className = "dialog-status error";
    refs.authDialogStatus.textContent = "Enter the email address to send the link to.";
    refs.authEmail.focus();
    return;
  }
  try {
    ...
    refs.authDialogStatus.className = "dialog-status";
    refs.authDialogStatus.textContent = "Sending link...";
    updateSyncUi("Sending sign-in link...");
    ...
    refs.authDialogStatus.textContent = "Link sent. Open the email on this device, then return here.";
    refs.authSubmitBtn.textContent = "Send again";
    updateSyncUi("Link sent · open the email and follow the magic link");
  } catch (error) {
    refs.authDialogStatus.className = "dialog-status error";
    refs.authDialogStatus.textContent = `Could not send link: ${error.message}`;
    updateSyncUi(`Could not send link: ${error.message}`);
  } finally {
    refs.authSubmitBtn.disabled = false;
  }
}
...
function ensureSharedCopyTargetLayoutId() {
  ...
  state.layouts[layoutId] = {
    id: layoutId,
    name: uniqueLayoutName("New layout"),
    rootContainerIds: [],
    arrangement,
...
async function openHistoryDialog() {
  if (isForcedOffline()) {
    showToast("History is unavailable in offline mode.", "error");
    return;
  }
  if (!currentUser) {
    showToast("History is available after sign-in.", "error");
    return;
  }
...
function updateRootContainerPlacementButton() {
  ...
  const currentText = isPackage
    ...
    : (active ? "Current layout" : "Outside current layout");
  refs.rootContainerPlacementField.hidden = false;
  if (refs.rootContainerPlacementLabel) refs.rootContainerPlacementLabel.textContent = "Located in";
  if (refs.rootContainerPlacementCurrent) {
    refs.rootContainerPlacementCurrent.hidden = false;
    refs.rootContainerPlacementCurrent.textContent = currentText || "Outside layout";
    refs.rootContainerPlacementCurrent.classList.toggle("active", active);
  }
  refs.rootContainerPlacementBtn.textContent = "Move within layout";
  ...
  refs.rootContainerPlacementBtn.setAttribute("aria-label", isPackage
    ? `Move from ${currentText || "current place"}`
    : `Move: ${currentText}`);
}
...
function updateRootContainerRemoveFromLayoutButton() {
  ...
  refs.rootContainerRemoveFromLayoutBtn.textContent = isNested ? "Delete forever" : "Remove from layout";
  refs.rootContainerRemoveFromLayoutBtn.setAttribute(
    "aria-label",
    isNested ? "Delete nested bag or place forever" : "Remove bag or place from layout"
  );
  refs.rootContainerRemoveFromLayoutBtn.title = isNested ? "Delete forever" : "Remove from layout";
...
async function confirmRemoveEditingContainerFromActiveLayout(event) {
  ...
  const nestedSubject = "This nested pouch will be deleted forever.";
  const confirmed = await askConfirmDialog({
    title: isRoot ? "Remove from layout?" : "Delete forever?",
    text: isRoot
      ? `“${container.name}” will be removed from the current layout.`
      : `“${container.name}” will be deleted forever as a nested bag/place.`,
    ...
    okText: isRoot ? "Remove" : "Delete forever"
  });
...
function placeRootContainerInActiveLayout(containerId, slotIndex) {
  ...
  showToast("Place selected. Click “Save”.", "success");
}
...
function renderCategoryPicker(target, selected = null, { fallbackDefault = true, idPrefix = "category" } = {}) {
  if (!target) return;
...
function renderRootContainerCategoryPicker(selected = null, { fallbackDefault = true } = {}) {
  renderCategoryPicker(refs.rootContainerCategoryList, selected, { fallbackDefault, idPrefix: "root-container-category" });
}
...
function updateItemContainerPickerButton() {
  ...
  const path = hasContainer ? layoutContainerPath(layout, containerId) : "Outside layout";
  if (refs.itemContainerLabel) refs.itemContainerLabel.textContent = hasContainer ? "Stored in" : "Place in";
  ...
  refs.itemContainerPickerBtn.textContent = "Move within layout";
...
function capturePackingScroll() {
  ...
  pendingPackingScroll = {
    ...
    bike3dDetail: isBike3dPackingView(packingViewMode) ? captureBike3dDetailViewport(refs.packingView) : null
  };
  if (!packingHidden) {
...
function restorePendingPackingScroll(board) {
  ...
  const { boardLeft, windowX, windowY, bike3dDetail = null } = pendingPackingScroll;
  pendingPackingScroll = null;
  lastPackingScrollSnapshot = { boardLeft, windowX, windowY, bike3dDetail };
...
function hasActiveContentFilter(view = getCurrentView()) {
  return Boolean(refs.searchInput.value.trim() || refs.locationFilter.value || selectedCategoryFilters.length);
}
...
function confirmRemoveItemFromActiveLayout(itemId) {
  ...
  openConfirmDialog({
    ...
    text: `“${item.name}” will disappear from the current layout, but will stay on the Items tab as unused.`,
    highlightText: "This item is currently used in the current layout.",
    okText: "Remove",
    tone: "danger",
...
function uniqueLayoutName(baseName = "New layout", { exceptLayoutId = "" } = {}) {
  const existingNames = Object.values(state.layouts || [])
    .filter((layout) => layout?.id !== exceptLayoutId)
    .map((layout) => layout?.name);
  return uniqueName(baseName, existingNames, { fallback: "New layout" });
}
...
function openLayoutDialog() {
  if (refs.layoutCreateTitle) {
    refs.layoutCreateTitle.textContent = canOpenAdminPublishedEdit() ? "New layout/template" : "New layout";
  }
  ...
  refs.layoutName.value = uniqueLayoutName("New layout");
