...
function applyLoadedStateToCurrentScope(nextState, { createFallbackLayout = true } = {}) {
  ...
  restoreAdminTemplatePhotoEditorNamespaces(exactAdminPhotoEditors);
  hydrateLocalSharedTemplateCatalogFromState(state);
}
...
function loadState({ createFallbackLayout = true } = {}) {
  ...
  try {
    ...
    if (preservePhotoEditors) {
      ...
      parsed = recoverPersonalAdminDrafts(parsed, mirror, adminRecovery);
    }
    installRuntimeActiveLayoutId(parsed, parsed.activeLayoutId);
    // The exact administrative mirror already exists. Rewriting it here could
    // evict unrelated recovery data through the legacy quota fallback.
    if (!preservePhotoEditors) persistStateSnapshot(parsed, { recordAction: false });
    return parsed;
  } catch (error) {
...
function saveBaseState(nextState = state) {
  return writeLargeScopedLocalValue(BASE_STATE_KEY, JSON.stringify(nextState), {
    clearRecovery: !hasOwnedAdminTemplatePhotoEditor(state)
  });
}
...
function saveRecoverySnapshot(reason, snapshot = state) {
  try {
    ...
    if (hasOwnedAdminTemplatePhotoEditor(snapshot)) return;
...
function solidifyTemplateDraftLayout(layoutId) {
  if (hasOwnedAdminTemplatePhotoEditor({ layouts: { [layoutId]: state.layouts?.[layoutId] } })) return false;
...
function solidifyManagedTemplateDrafts() {
  if (hasOwnedAdminTemplatePhotoEditor(state)) {
    ...
    let changed = false;
    for (const layoutId of Object.keys(state.layouts || {})) changed = solidifyTemplateDraftLayout(layoutId) || changed;
    return changed;
  }
...
async function applyAdminTemplatePhotoCreateCandidate(record) {
  ...
  try {
    ...
    await persistAdminTemplatePhotoMirror(layoutId, [expectedNamespace, adminTemplatePhotoNamespace(state, layoutId)]);
  } catch (error) {
...
async function captureAdminTemplatePhotoCreateForm(input, { isCurrent, onDurable }, captureComplete = () => {}, captureLease) {
  ...
  try {
    ...
    onDurable(record); durable = true; guard(); await applyAdminTemplatePhotoCreateCandidate(record); captureComplete();
...
async function resumeAdminTemplatePhotoCreateForm(layout, captureLease) {
  ...
  await applyAdminTemplatePhotoCreateCandidate(record);
}
...
// The acceptance marker is small and synchronous; the account mirror is read
// through its current repository and written through an explicit async API.
// Never emulate an async Storage.setItem for the large mirror.
const adminTemplatePhotoMirrorStorage = Object.freeze({
  getItem: key => readPersonalLocalValue(key),
  setItem(key, raw) {
    if (ownsPersonalMirror(key)) throw Error("Зеркало данных требует подтверждённой асинхронной записи.");
...
async function readAdminTemplatePhotoWholeCopyStopped(binding, layoutId, operationId, guard, preparing = true) {
  const getContext = () => adminTemplateOperationContext(binding, layoutId, preparing);
  const store = createAdminTemplatePhotoWholeCopyActionStore({ binding, getContext, enabled: false });
  return readAdminTemplatePhotoWholeCopyCancelled({ binding, operationId, store, getContext, storage: localStorage }, guard);
}
...
async function withAdminTemplatePhotoWholeCopyInventoryScope(proof, task, phase) {
  ...
  const rawGuard = () => {
    ...
    if (raw !== validatedJournalRaw) {
      ...
      if (canonicalTemplateJson(row) !== raw || !same(row.intent, intent) || row.recordIntentHash !== record.intentHash
        || row.cancelRequested === true && !(phase === "apply" && row.receipt?.operation.state === "committed")) pause();
  ...
  try {
    ...
    for (const retained of actorRecords.records) {
      ...
      if (sides.some(side => same(side.binding, retained.binding) || side.binding.listId === origin.listId && side.binding.itemKey === origin.itemKey)) {
        const stopped = await readAdminTemplatePhotoWholeCopyStopped(retained.binding, record.snapshot.source.layoutId,
          retained.action.operationId, rawGuard, true); guard();
        if (stopped && same(stopped.record, retained) && !sides.some(side => same(side.binding, retained.binding))) {
          stoppedWhole.set(retained.action.operationId, stopped); guard(); continue;
        }
...
async function findAdminTemplatePhotoWholeCopyFormRecord(layoutId, operationId = null) {
  ...
  if (operationId === null) {
    ...
    for (const record of selected) {
      ...
      const stopped = !accepted && await readAdminTemplatePhotoWholeCopyStopped(record.binding, layoutId, record.action.operationId, guard, true); guard();
      if (!accepted && !stopped) pending.push(record);
    }
    selected = pending;
  }
...
async function resumeAdminTemplatePhotoWholeCopyForm(layoutId, operationId, { onProgress = null } = {}) {
  ...
  if (journal?.cancelRequested) {
    // Automatic continuation may only inspect the original stop. Repeating a
    // cancellation POST requires the explicit recovery-dialog action below.
    const facts = await adminTemplatePhotoWholeCopyRecoveryRunner(binding, layoutId).inspect(operationId); guard();
    if (facts.receipt?.operation.state === "committed") return applyAdminTemplatePhotoWholeCopyFormResult(facts);
    return { state: !facts.receipt ? "stopping" : facts.receipt.result.payload.code === "operation_cancelled" ? "stopped" : "rejected", operationId };
  }
  if (!saved || !journal) {
...
function adminTemplatePhotoWholeCopyRecoveryRunner(binding, layoutId) {
  const getContext = () => adminTemplateOperationContext(binding, layoutId);
  const store = createAdminTemplatePhotoWholeCopyActionStore({ binding, getContext, enabled: false });
  return createAdminTemplatePhotoWholeCopyRecoveryRunner({ binding, layoutId, getContext, store,
    plans: adminTemplatePlansFor(binding, layoutId, true), storage: localStorage, locks: navigator.locks,
    createClient: options => createAdminTemplatePhotoWholeCopyClient({ ...options, transport: experimentTransport,
...
async function prepareAdminTemplatePhotoWholeCopyRecovery(layoutId) {
  ...
  const describe = journal => {
    ...
    return { recoveryKind: "photo-whole-copy", id,
      operations: [{ id, kind: "template.copy", state: receipt?.operation.state || (journal?.dispatched ? "unknown" : "queued"), cancelled }],
      stopRequested: stopping, stopCoversHead: stopping, stopped: cancelled, committedCount: committed ? 1 : 0, applied,
      canResume: !applied && (committed || !receipt && (stopping ? adminTemplateUiEnabled() : adminTemplatePhotoWholeCopyFormEnabled())),
      canStop: Boolean(journal) && !receipt && !stopping && adminTemplateUiEnabled(), canCompare: false };
  };
  const inspect = async refresh => {
    ...
    latest = journal; inspected = true; return describe(journal);
  };
...
async function applyAdminTemplatePhotoWholeCopyFormResult(input) {
  ...
  return withAdminTemplateCapture({ bindings, locks: navigator.locks }, async captureLease => {
    ...
    if (!saved || saved.cancelRequested || canonicalTemplateJson(saved.plan) !== canonicalTemplateJson(plan)
      || !journal || canonicalTemplateJson(journal.receipt) !== canonicalTemplateJson(receipt)
    ...
    const getMirrorContext = () => ({ storage: adminTemplatePhotoMirrorStorage, key: scopedLocalStorageKey(STORAGE_KEY), scopeKey: localStorageScopeKey });
    ...
    return withAdminTemplatePhotoWholeCopyApplyInventory({ record, bindings, captureLease, assertCurrent: guard }, scope =>
      applyAdminTemplatePhotoWholeCopyResult({ plan, store, receipt, stageReceipts, captureLease, getState: () => state, getContext, persistMirror: persistRequiredPersonalMirror,
...
async function applyAdminTemplatePhotoCopyPending(record) {
  ...
  try {
    ...
    layout.templateDraftSyncPending = true;
    await persistAdminTemplatePhotoMirror(layoutId, [expected, adminTemplatePhotoNamespace(state, layoutId)]);
  } catch (error) {
...
async function resumeAdminTemplatePhotoCopyForm(layout) {
  ...
  await withAdminTemplateCapture({ bindings: [binding, sourceBinding], locks: navigator.locks }, async captureLease => {
    ...
    await applyAdminTemplatePhotoCopyPending(record);
  });
}
async function submitAdminTemplatePhotoCopyForm(input, { isCurrent, onDurable }) {
  ...
  await withAdminTemplateCapture({ bindings, locks: navigator.locks }, async captureLease => {
    ...
    await applyAdminTemplatePhotoCopyPending(record); captured = true; onDurable(record);
  });
...
async function assertAdminTemplateCopyCaptureAllowed(binding, layoutId,
  ...
  for (const record of wholeInventory.records) {
    ...
    if (!isTarget && operationId !== record.action.operationId && Number.isSafeInteger(body.base?.stateRevision)
      && body.base.stateRevision >= source.base.stateRevision && !body.base?.operationId) {
      const stopped = await readAdminTemplatePhotoWholeCopyStopped(record.binding, layoutId, record.action.operationId, contextGuard, true); check();
      if (stopped && canonicalTemplateJson(stopped.record) === canonicalTemplateJson(record)) {
        acceptedProofs.push(stopped); check(); continue;
      }
    }
...
async function adminTemplatePhotoSourcePayload(layout) {
  ...
  if (baseline?.stateRevision === source.base.stateRevision) return baseline.payload;
  ...
  const prepared = await adminTemplateClient(source.binding, layout.id).prepare();
...
async function resumeAdminTemplatePhotoAppendForm(layout, captureLease) {
  ...
  const guard = () => {
    if (state.layouts[layout.id] !== layout || layout.adminCausalSource !== source
      || canonicalTemplateJson(adminTemplateOperationContext(binding, layout.id)) !== initial
      || canonicalTemplateJson(adminTemplatePhotoNamespace(state, layout.id)) !== original) throw Error("Контекст восстановления фотопакета изменился.");
  };
  const store = adminTemplatePhotoStore(binding, layout.id), pending = [];
  ...
  if (!pending.length) {
    if (source.photoAppendPending) throw Error("Исходный фотопакет не найден. Сохранение остановлено для сверки.");
    return;
  }
...
async function captureAdminTemplatePhotoAppendForm(input, { isCurrent, onDurable }, captureComplete = () => {}, captureLease) {
  ...
  let attempt = administrativePhotoAttempts.get(input);
  if (!attempt) {
    attempt = { operationId: crypto.randomUUID(), beforeState: adminTemplatePhotoNamespace(state, layoutId), initial, record: null };
    administrativePhotoAttempts.set(input, attempt);
  }
  ...
  if (attempt.initial !== initial) throw Error("Контекст исходной формы изменился. Фотопакет сохранён для сверки.");
  let durable = false;
  const guard = () => {
    if (state.layouts[layoutId] !== layout || layout.adminCausalSource !== source
      || canonicalTemplateJson(adminTemplateOperationContext(binding, layoutId)) !== initial
  ...
  try {
    ...
    const ownerMap = captureAdminTemplatePhotoOwnerMap({ binding, layoutId, stateRevision: source.base.stateRevision,
      sourcePayload, state, mappings: photoOwners });
    const editor = adminTemplateEditorSnapshot(layoutId, { photoOwners });
    // Keep the complete raw source outside this form's selected fields. The
    // ordinary exporter may normalize other owners or template arrangements.
    const payload = adminTemplatePhotoFormPayload({ sourcePayload, ownerMap, entityType: input.entityType, entityId: input.entityId, fields });
    guard();
...
function adminTemplateCanonicalEditorSnapshot(layoutId) {
  ...
  // A public copy source now also has an exact owner map. Serialize its view
  // without the legacy exporter's temporary remapping of live state. This is
  // a detached snapshot, not permission to publish or change source files.
  if (!(source.visibility === "private" || source.visibility === "public" && adminTemplatePhotoWholeCopyFormEnabled())
    || !source.exists || map.layoutId !== layoutId
...
function adminTemplateEditorSnapshot(layoutId, options = {}) {
  ...
  return withLayoutArrangementApplied(layoutId, () => {
    ...
    for (const type of ["items", "containers"]) Object.assign(preservedEntityIds[type], options.photoOwners?.[type] || {});
...
async function openCausalAdminTemplate(target, { remember = true } = {}) {
  try {
    ...
    if (existing) {
      ...
      await resumeAdminTemplatePhotoForm(existing);
