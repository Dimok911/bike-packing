...
function isMeaningfulPackingState(targetState = state) {
  return Boolean(
    targetState &&
...
function isPackingStateShape(targetState) {
  return Boolean(
    targetState &&
...
function stateStats(targetState = state) {
  const containers = targetState?.containers && typeof targetState.containers === "object" ? targetState.containers : {};
  const items = targetState?.items && typeof targetState.items === "object" ? targetState.items : {};
...
function normalizeIntegrityCount(value) {
...
function stateIntegrityMetaFromResponse(...sources) {
  const result = {
    payloadHash: null,
    entityHash: null,
    itemCount: null,
...
function hasStateIntegrityMeta(meta) {
  return Boolean(meta && (
    meta.payloadHash ||
    meta.entityHash ||
    meta.itemCount !== null ||
...
function remoteStateIntegrityError(remoteState, meta, rawPayload = null) {
  ...
  error.meta = meta;
  error.stats = stats;
  if (mismatches.length) return error;
  return null;
}

function isDangerousRemoteIntegrityMismatch(stats, meta) {
...
function compactPhotoForSync(photo) {
...
function annotatePayloadError(error, report) {
...
function stripItemPlacementFields(record) {
...
function stripContainerArrangementFields(record) {
  if (!record) return record;
