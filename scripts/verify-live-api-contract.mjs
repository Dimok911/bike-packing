...
const capabilitiesUrl = `${API_BASE.replace(/\/$/, "")}/bike-packing/capabilities`;
...
try {
  ...
  console.log(`Live API contract verified: ${liveVersion}; ${REQUIRED_ADMIN_API_CAPABILITIES.length} required capabilities.`);
} catch (error) {
  console.error(`Production frontend/API contract check failed at ${capabilitiesUrl}: ${error.message}`);
  process.exitCode = 1;
} finally {
